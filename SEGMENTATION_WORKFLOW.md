...
5. 水平断をクリックして3方向照合の交点を移動し、同じ選択ラベルと未保存差分を冠状断・矢状断で確認します。
6. 変更範囲、判断根拠、参照文献、確認者をメモします。
7. 「差分JSONを書き出す」で保存し、Pull Requestへ添付します。
...
- `reviewer` / `reviewedAt`: ブラウザからの書き出し時は空欄。確認者がPull Requestで採否を決めた後だけ記録
- `proposedLabels`: 差分が新たに指定するラベルIDと名称。背景化だけの場合は空配列
- `affectedHorizontalSlices`: 差分の全Z番号と最小・最大値。`runs` から自動生成され、適用時にも一致を検証
...
- 使用した差分JSON
- 自己レビューの有無、可能なら第二確認者
- アプリ内表示だけで作成した変更前・変更後の同一断面スクリーンショット
- 確認者、採用・要修正・差し戻しの判断、判断日、要修正／差し戻しの場合は理由
...
出力後に各ラベル数、左右、空間的位置、隣接断面での連続性、画像組織との重なり、クイズ対象断面を再検証します。差分JSON、監査結果、同一断面の変更前後比較、確認者、採否または差し戻し理由をPull Requestに残します。公式配布ファイルへの置換はプロジェクト管理者の採用決定後に行います。

## 現在の制限

- 主作業面は水平断のみです。冠状断・矢状断は交点と選択ラベルを照合する確認表示で、そちらから直接編集はできません。
