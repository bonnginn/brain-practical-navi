...
2026-09-05追補: 現行採用後ラベルの再計算と64断面の原画像照合は[脳室再点検](SEPTEMBER_VENTRICLE_REVIEW.md)に記録した。三軸挟み込み残件0、4面近傍候補4 voxelは縁にあり不採用。以下の2026-08-24の採用記録は書き換えない。
...
この採用段階の脳室ラベルは、CerebrAの脳室候補を同一0.5 mm BigBrain格子へ移し、BigBrain画像の背景符号255（元画像の65535相当）と重なる部分だけに制限している。ラベル内の非背景符号voxelは0である。ただし255は表示用の高値／非有限値を含む符号化であり、**それだけで組織不存在を確定できない**（2026-09-06説明補正）。脳室に固有の値でもなく、血管腔、裂、標本外背景、標本作製・位置合わせによる空隙も同じ値になり得る。採用済み33 voxelの原画像による個別判断と、この一般的な画像値の限界は区別する。
