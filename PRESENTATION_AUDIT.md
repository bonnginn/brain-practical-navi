...
QRと端末追加案内でHomeカードが画面より高くなる場合は、カードと下部注記を内容高に沿って縦に配置し、Home領域内でスクロールさせる。下部注記を固定高のgrid行へ残してカード上へ重ねない。修正後のローカルproduction previewをアプリ内ブラウザで確認し、970×545相当ではfooterと下部注記の間隔47.5 px・重なり0、390×768 override（実効295×582）では間隔44.7 px・重なり0・横overflow 0だった。
