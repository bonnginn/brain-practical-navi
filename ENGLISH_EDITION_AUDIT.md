...
- A dedicated English Google Form and response sheet were created on 2026-08-29 and the public responder URL is configured as `VITE_FEEDBACK_FORM_URL_EN`. Logged-out inspection confirmed one English feedback-only page, optional reply contact, and no file upload or collaborator-recruitment fields. Google Forms chrome such as Submit and Required follows each respondent's Google/browser locale and may therefore appear in Japanese on a Japanese-language device. A complete submit/delete lifecycle in both Forms and its response sheet remains untested.
