...
- [x] クイズでは正解対象が十分な面積で表示され、断面問題ではスライダーを動かせる（全断面問題の着色面積を自動検査し、スライダーに1断面ずつの送り／戻しを追加。通常幅・390 pxで実操作）。2026-08-24、断面17・脳表6・神経血管17 targetをPC／横向きタブレット／phoneの計120件で着色→解除→再着色し、Chrome 151の独立validatorで120/120件に合格した。半透明神経血管はdepth・描画順・selected色の残存を再現し、PCAも3幅すべてoutside 0。1 px許容・合格閾値は緩和していない。詳細は [QUIZ_TARGET_VISIBILITY_AUDIT.md](QUIZ_TARGET_VISIBILITY_AUDIT.md)。
...
- [ ] 脳神経と主要血管の同定問題を追加する。（部分完了: 既存模式3Dだけを使う17問のpilot（血管6・脳神経I、III–XII）を追加し、`neurovascular`／`arteries`／`cranialNerves`の候補数、wrong-only、復習リンク、由来台帳、overlay region ID、既存23問の別SHAを監査。画面上は「模式3D・専門家未確認」、問いは白色強調の名称同定のみ。Chrome 151のローカル本番preview `http://127.0.0.1:4196` で17問、白色強調、透過脳表、小脳OFF、誤答feedback、復習遷移、試作OFF候補0、WebGL fallback、390×768指定（実効innerWidth 295）の横overflowなし、console logs 0を確認し、全経路156/156件に合格。結果は `NEUROVASCULAR_QUIZ_AUDIT.md` と `work/browser-audit/beta-route-audit-neurovascular-quiz-final-2026-08-22.json` に固定。公開URL、物理端末、別GPU、専門家レビュー、細枝・静脈・連絡動脈、IIと視覚路の個別分節は未完了。）
...
- [ ] 3Dと断面を同期し、段階的な回路観察（再生、一時停止、前後移動）を試作する。（部分完了: 自由観察の大脳基底核回路へ、既存クイズ位置と7対象の手動分節ラベルだけを使う5段階・7対象の3D＋断面ステッパーを追加。前／次／再生／一時停止／最初へ戻る、プリセット切替・離脱時のタイマー停止、スマートフォン縦積みを実装し、画素監査と `app/page.tsx` のラベルID照合を `PATHWAY_STEPPER_AUDIT.md` と `scripts/audit_pathway_stepper.mjs` に固定。Chrome 151で操作確認し、最終ビルドの26経路×3幅×direct/reload＝156/156、全テスト134/134、型検査、通常／Pagesビルドに合格。完全回路、模式線、投射方向、興奮性／抑制性は表示しない。専門家レビュー、視覚路・Papez回路の同等ステッパーは未完了。）
...
追加確認（2026-08-23）: phone中心操作の最終保存結果は `work/browser-audit/phone-core-interactions-v16-2026-08-23.json` である。Windows 11／Node 24／Chrome 151、通常production preview `http://127.0.0.1:4250/`、390×768・DPR1・`mobile:true`・`touch:true`・最大同時タッチ5・縦向きで、dock／脳表・左外側面／水平断／復習の4 journeyを実タッチイベント列で確認し、`allPassed: true`、独立validator failure 0だった。Solレビュー後のvalidatorは、action summaryとprobeの対応、surface target keyと選択状態の連続性、wrong-answerからreview-linkまでのquestion target連続性、touch targetのgeometry・primaryTouchId・target/touch ID対応、tapのtouchPoints 1→0／dragの1→1→0、実設定遷移を独立に再計算するよう補強した。脳表は初期の内側面から設定sheetの実タッチで左外側面へ、断面は初期の矢状断から実タッチで水平断へ移り、no-op操作として扱っていない。これはcoarse-touch emulationの部分確認であり、スマートフォンUI全体の完了扱い、物理端末・実機タッチ、公開URL、Safari・別ブラウザ、別GPU、専門家レビューの確認を意味しない。）
- [x] 公開データ量と初回表示時間を実測し、過大な取得を減らしている。（2026-08-24、M2比較専用URL追加後のcanonical 27経路cold payload監査は27/27件、route監査は27経路×3幅×direct/reload＝162/162件。通常の共同制作ページは比較資産を取得せず、専用URLだけが比較用3資産のexact allowlistに一致した。2026-08-23のpial gzip性能suite 37/37件、圧縮pial物理パスのみ、raw要求0件、Home・static経路のatlasなし、surface／sections／blocks／segment／quizのexact初回allowlistとartifact-derived budgetも維持した。stable-time回帰は1%未満、sampledPeak backing storage最大増加は2.0%。`public/` 全体はPWA用PNGアイコン追加後92,446,938 B（88.16 MiB）で100 MiB未満、sectionsは26,441,013 Bで旧34,688,033 B（34.69 MB）から23.8%減り、20–30 MB暫定目安内。結果は [PERFORMANCE_AUDIT.md](PERFORMANCE_AUDIT.md)、`work/performance/initial-route-payload-model-strategy-discovery-2026-08-24.json`、`work/browser-audit/beta-route-audit-model-strategy-discovery-2026-08-24.json`、`work/performance/performance-suite-pial-gzip-2026-08-23.json` に記録。公開URL・公開回線・物理端末・別GPU／別ブラウザは未確認。）
- [ ] 自動テスト、本番ビルド、公開URLの全経路巡回に合格する。（2026-08-24、M2比較専用URLを含むcanonical route監査で27経路×3幅×direct/reload＝162/162件をWindows Chrome 151（`http://127.0.0.1:4312`）で確認。`work/browser-audit/beta-route-audit-model-strategy-discovery-2026-08-24.json` に固定し、missing/duplicate/fail=0、console/request/UI error・loader・overflow・WebGL fallback=0。公開URL反映確認待ち。2026-08-23の26経路156/156件と2026-08-22のstatus追加前25経路150/150件は履歴として保持。）
- [ ] クイズ対象は画面上で十分に確認でき、未確認構造を正答として要求しない。（粒度監査で既存23問を標準7件・試作16件、模式3D pilot 17件の一意な40件として形式・詳細・トピック・由来へ分類し、targetと全選択肢の由来を照合。provisional選択肢を含むpallidum・accumbens・hippocampus・mammillaryBodyを試作へ分類し、試作OFF時の候補除外、標準回答セットの全件解決、unknown由来の拒否を自動検査。Chrome 151のローカルpreviewで試作ON時40件（標準7・試作33）、OFF時7件（標準7・試作0）、標準queue、乳頭体問題の試作バッジを実操作し、最終route監査156/156件に合格。旧ID33の正答対象外、誤答から観察画面への復帰も確認済み。標準採用基準の解剖学的妥当性と未確認構造が選択肢へ現れないことの専門家確認は未完了。）
...
- [PHONE_CORE_INTERACTION_AUDIT.md](PHONE_CORE_INTERACTION_AUDIT.md): coarse-touch phoneの中心操作4 journeyと未確認範囲
