...
- [ ] 脳神経と主要血管の同定問題を追加する。（部分完了: 既存模式3Dだけを使うpilotを22問（血管9・神経13）へ拡張した。2026-08-28に前交通動脈、後交通動脈、小脳動脈群、視交叉を追加し、全45問とした。`cn2`はoverlay ID23・24、`opticChiasm`はID25だけを使い、旧断面ID33と未分節ID36–38を含めない。問いは白色強調の名称同定だけで、全問「模式3D・専門家未確認」。脳神経問題では大脳半球を透過しつつ中脳・橋・延髄を不透明な起始位置目安として残す。2026-08-24の41 target×3幅＝123/123件は追加前の履歴として保持する。公開URL、物理端末、別GPU、専門家レビュー、細枝・静脈、視覚路の個別分節は未完了。）
- [ ] 断面方向・脳表・脳底・深部構造など、項目別出題の粒度を整える（部分完了: 45表示対象の名称同定に、機能・位置関係・経路を問う55問を追加して全100問とした。追加問題は `QUIZ_CONCEPT_BANK_AUDIT.md` で独立監査し、専門家未確認の試作として扱う。トピックと独立した形式・詳細フィルタ、由来区分、候補数連動の問題数、候補0件説明、wrong-only、復習リンクを実装。45 targetの可視性はローカルChromeで135/135件確認済みだが、追加55問すべての専門家レビュー、公開URL、物理端末、別GPUは未完了。）
...
- [x] 既存Google Formを自動更新せず、機械可読契約から生成した契約descriptorで、列挙済みの設定・説明・質問・help text・選択肢・「その他」・分岐・運用シート見出しを読み取り専用で照合し、差分時に停止するpreflightを用意する（ローカル監査のみ完了。現行外部フォームへの適用、回答送信・削除運用、管理者確認は未完了）。
...
| M2 3D表示改善（β後へ継続） | 現行MNI脳表で、領域着色時にも脳回・脳溝の立体感を自然に保つ描画方法を検討する | 新規3Dモデルの制作は当面行わず、通常教材へ採用しない比較試作とレビュー記録から描画方式を選ぶ |
| M3 β候補版 | 必修範囲、クイズ、権利、共同編集、公開運用を統合 | 下記のGo/No-Go確認にすべて合格する |

2026-08-30方針: 新規の脳表ポリゴンモデル案と、中心溝ガイドを含む着色A/B試作は、自然さの改善を十分に示せなかったため撤回した。現行MNI脳表を継続使用し、課題は「新規メッシュ制作」ではなく「選択色を重ねても元の照明、脳回の丸み、脳溝の暗さを損なわない色合成」としてβ後の検討事項に残す。β候補版の公開条件にはしない。

## β版 Go / No-Go

- [ ] 必修構造は、検証済みか、模式・推定であることが画面上で判別できる。（部分的な根拠: 由来区分・表示面・既知の制限を監査索引へ記録し、既存23件と模式3D pilot 22件の一意な45件の正答対象をアプリ実装と照合した。画面全必修範囲のGo/No-Go判定は未実施。）
...
追加確認（2026-08-24）: phone中心操作の最終保存結果は `work/browser-audit/phone-core-interactions-v18-focus4-guided-2026-08-24.json` である。Windows 11／Node 24／Chrome 151、通常production preview `http://127.0.0.1:4330/`、390×768・DPR1・`mobile:true`・`touch:true`・最大同時タッチ5・縦向きで、dock／脳表・左外側面／水平断／復習／β重点4ブロック標本ガイドの5 journeyを実タッチイベント列で確認し、`allPassed: true`、独立validator failure 0だった。Solレビュー後のvalidatorは、action summaryとprobe、touch geometry・primaryTouchId・target/touch ID、実設定遷移に加え、β重点4の固定順、fresh route、合計17 single-layer段階、4 final all、段階番号、終了後復元を独立に再計算する。これはcoarse-touch emulationの部分確認であり、スマートフォンUI全体の完了扱い、解剖学的順序の検証、物理端末・実機タッチ、公開URL、Safari・別ブラウザ、別GPU、専門家レビューの確認を意味しない。）
...
- [ ] クイズ対象は画面上で十分に確認でき、未確認構造を正答として要求しない。（粒度監査で既存23問を標準7件・試作16件、模式3D pilot 22件の一意な45件として形式・詳細・トピック・由来へ分類し、targetと全選択肢の由来を照合。provisional選択肢を含むpallidum・accumbens・hippocampus・mammillaryBodyを試作へ分類し、試作OFF時の候補除外、標準回答セットの全件解決、unknown由来の拒否を自動検査。追加した前交通動脈・後交通動脈・小脳動脈群・視交叉も模式3D・専門家未確認のまま扱い、視交叉はoverlay ID25だけを使用する。脳神経問題では大脳半球を透過しつつ中脳・橋・延髄を不透明な起始位置目安として残す。旧ID33は正答対象外で、標準採用基準の解剖学的妥当性と未確認構造が選択肢へ現れないことの専門家確認は未完了。）
