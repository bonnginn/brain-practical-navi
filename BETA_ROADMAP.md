...
- [ ] 実習で頻出する標本から優先し、切り出しの意味と対応を確認者が記録する。（部分完了: 8標本の実装契約・既存データ範囲・未確認事項を `BLOCK_CONTEXT_AUDIT.md` に記録。8標本のCanvas生成／破棄、代表断面切替、標本切替時の初期OFF・全脳表示復帰、フォーカス復帰、小画面のエラー・横はみ出しなしを確認済み。専門家レビューと追加7標本の保存済み性能値は未完了。）
