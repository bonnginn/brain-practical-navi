...
- [x] 小画面でも主要操作が画面外へ消えず、デスクトップでは観察領域を不必要に縮めない（761–1199 pxの狭幅PCでは補助操作と6教材ナビを重ならない二段ヘッダーへ分離。さらに `hover: none`・`pointer: coarse` のタッチ主体端末では6ワークスペース下部ドックと画面別設定シートを使う専用Web UI。PCの分割表示やズームだけでは電話UIへ切り替えない）。
...
- [ ] 圧縮後転送量、ブラウザキャッシュ量、初回表示時間、メモリ使用量をPCとスマートフォンで計測する（PCと390 px相当の実ブラウザ計測、経路別静的予算、小画面3D遅延後の再測定まで完了。左右高密度脳表を可逆圧縮し、公開物70.6 MiB・デスクトップ水平断23.6 MiBへ削減。schema v5診断・schema v2性能セッションで公開HTTPS初回ホームと10経路の取得量・Canvas・overflow・取得可能なJS heapを採取でき、History API移動後も直前教材URLを観測へ結び付ける。実スマートフォンのCPU・GPU・回線・OS/GPUを含むピークメモリ証拠は未完了）。
