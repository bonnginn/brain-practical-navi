...
進捗: 方法検証用 `PROTOTYPE 01` として、後脳を `/?m2=compare#workspace/blocks/hindbrain` で同条件比較できるようにしました。標本再構成と模式補助を別Canvas・別由来表示で同期回転し、[MODEL_COMPARISON_AUDIT.md](MODEL_COMPARISON_AUDIT.md) に評価を記録しています。β基盤は標本再構成、模式形状は着脱可能な補助とする暫定判断です。ロードマップ優先例の内側側頭葉／脳室全景で未検証形状を新造しておらず、専門家・学習者比較も未実施のためM2全体は継続します。
