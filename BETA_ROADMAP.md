...
- [x] 脳室全景、レンズ核と投射線維、脈絡叢、内側側頭葉など、切り出す意味が明確な標本から改善する（4標本の初期構図・透過組織・レイヤー着脱を実ブラウザ確認。投射線維は核・内包・放線冠の5/7、内側側頭葉は標本分節・試作腔の3/5から始め、追加の模式補助を段階表示。小画面で固定ナビに隠れていた視点ボタンも退避）。
