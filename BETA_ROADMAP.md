...
- [ ] 圧縮後転送量、ブラウザキャッシュ量、初回表示時間、メモリ使用量をPCとスマートフォンで計測する（PCと390 px相当の実ブラウザ計測、経路別静的予算、小画面3D遅延後の再測定まで完了。左右高密度脳表を可逆圧縮し、公開物70.6 MiB・デスクトップ水平断23.6 MiBへ削減。schema v4で公開HTTPS初回ホームと10経路の取得量・Canvas・overflow・取得可能なJS heapを採取できる。実スマートフォンのCPU・GPU・回線・OS/GPUを含むピークメモリ証拠は未完了）。
