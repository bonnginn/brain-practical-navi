...
- [ ] 圧縮後転送量、ブラウザキャッシュ量、初回表示時間、メモリ使用量をPCとスマートフォンで計測する（PCと390 px相当の実ブラウザ計測、経路別静的予算、小画面3D遅延後の再測定まで完了。左右高密度脳表を可逆圧縮し、公開物70.6 MiB・デスクトップ水平断23.6 MiBへ削減。実スマートフォンのCPU・GPU・回線・ピークメモリが未完了）。
...
- 診断JSONをschema v3へ更新し、画面移動・PWA再起動後も端末内下書きを復元する。オンライン6経路、機内モード4経路、保存後オンライン・機内モード起動・再接続後の3チェックポイントに分け、単独起動、Service Worker制御、同一カタログ、3教材セットの実キャッシュ完全性を自動記録する。`npm run validate:device-check -- <record.json>` は実タッチ、10/10完走、3段階の順序・接続・保存欠落を検出するが、実機Gateを自動承認しない。
