...
`public/atlas/structure-provenance.json` を単一情報源として、`expertReview: "pending"` の項目を共同制作画面へ読み取り専用で表示します。これは専門家レビューの完了、解剖学的妥当性、公開採否を示すものではありません。台帳の項目・ラベル・境界を編集、承認、採否する操作はありません。

2026-08-24、後日の確認を中断・再開できるよう、各カードへ1項目単位の「確認記録の端末内下書き」を追加しました。氏名・連絡先・所属・自由記述は記録せず、固定3観察項目と固定懸念コードだけを保存します。台帳全体と対象entryのSHA-256が一致しない古い下書き、JSON不正、保存障害、別タブ競合はロックし、別項目へ自動移行しません。書き出しJSONも未提出・未採用・専門家確認未主張です。詳細は [ANATOMY_REVIEW_RECORD_DRAFT_AUDIT.md](ANATOMY_REVIEW_RECORD_DRAFT_AUDIT.md) を参照してください。
...
監査は、pending項目のstable keyの重複・欠落、representation／learner surface／レビュー状態、source refsの解決、ID33除外、ID39・40のプロジェクトレビュー済み／専門家pending、読み取り専用台帳UI契約を検査します。端末内下書きの形式・現在台帳SHA・固定選択肢・未提出／未採用sentinelは `scripts/audit_anatomy_review_record_draft.mjs` で独立検査します。ID33の客観的な直交断情報は別途 `python scripts/audit_optic_orthogonal.py` で再生成し、保存JSONとの完全一致、不正SHA・寸法・内容改変の拒否を自動テストします。
