...
後続の原画像確認でcn5も保留へ追加。現在は保存100問・出題92問（保留は同定4＋機能4）です。以下の94問はIX〜XIのみを保留した中間段階。問題データは保持し、geometry修正は未完了です。

2026-09-06 開発版・未公開：以下の100問は保存bankの数です。IX〜XI模型の配置不一致により、名称同定3問と模型依存の機能3問を保留し、出題可能数は94問です。問題本文や正答データは保持しています。監査結果のsummaryは保存数、eligibilityは出題可能数を表し、runtime保留filter欠落も検出します。これは問題数拡張の取り消しや神経geometryの修正完了ではありません。NERVE_ORIGIN_IMAGE_REVIEW.md参照。
