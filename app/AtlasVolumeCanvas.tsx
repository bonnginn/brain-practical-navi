...
const QUIZ_VESSEL_ID_OFFSET=1000,QUIZ_NERVE_ID_OFFSET=2000;
...
function loadMesh(name:string){
  const file=name==="pial-left"||name==="pial-right"?`${name}.mesh.gz`:`${name}.mesh`;
  if(!meshCache.has(name))meshCache.set(name,fetch(`${ASSET_BASE}atlas/${file}`).then(r=>{if(!r.ok)throw new Error(`${name} HTTP ${r.status}`);return r.arrayBuffer()}).then(async compressed=>{
    let buf=compressed;
    if(new DataView(buf).getUint16(0,false)===0x1f8b){const stream=new Blob([buf]).stream().pipeThrough(new DecompressionStream("gzip"));buf=await new Response(stream).arrayBuffer()}
...
export function AtlasVolumeCanvas({kind,plane,position,focus,display,rotation,view="inside",contrast="t1",highlights=[],surfaceHighlights=[],surfaceLandmarks=[],surfaceDeepLandmarks=[],neurovascularHighlights=[],specimenLayers=[],specimenTissueMode="solid",selectionMeshLayers=[],onIdentify,onSurfaceIdentify,onViewChange,showFocus=true,showCutPlane=true,showZoomControls=true,hemisphere="both",showCerebellum=true,showPonsMedulla=true,showMidbrain=true,specimenBlock="none",neurovascularOverlay="none",showBrainstemNerves=true,showBasalLandmarks=false,basalLandmark="all",basalHighlights=[],basalOnlySelected=false}:{kind:"surface"|"slice";plane:Plane;position:number;focus:Focus;display:Display;rotation:Rotation;view?:"inside"|"ghost"|"extracted"|"segmented";contrast?:"t1"|"t2"|"bigbrain"|"single";highlights?:HighlightLayer[];surfaceHighlights?:HighlightLayer[];surfaceLandmarks?:SurfaceLandmark[];surfaceDeepLandmarks?:SurfaceDeepLandmark[];neurovascularHighlights?:HighlightLayer[];specimenLayers?:string[];specimenTissueMode?:SpecimenTissueMode;selectionMeshLayers?:SelectionMeshLayer[];onIdentify?:(point:IdentifiedPoint)=>void;onSurfaceIdentify?:(point:SurfaceIdentifiedPoint)=>void;onViewChange?:()=>void;showFocus?:boolean;showCutPlane?:boolean;showZoomControls?:boolean;hemisphere?:"both"|"left"|"right";showCerebellum?:boolean;showPonsMedulla?:boolean;showMidbrain?:boolean;specimenBlock?:SpecimenBlock;neurovascularOverlay?:NeurovascularOverlay;showBrainstemNerves?:boolean;showBasalLandmarks?:boolean;basalLandmark?:BasalLandmark;basalHighlights?:BasalLandmark[];basalOnlySelected?:boolean}){
  const initialZoom=useRef(kind==="surface"&&typeof window!=="undefined"&&window.matchMedia("(max-width: 760px) and (hover: none) and (pointer: coarse)").matches?.87:1),ref=useRef<HTMLCanvasElement>(null),panDrag=useRef<{x:number;y:number;pan:{x:number;y:number}}|null>(null),surfaceClick=useRef<{x:number;y:number;moved:boolean}|null>(null),sliceTouchClick=useRef<{x:number;y:number;moved:boolean}|null>(null),pinch=useRef<{distance:number;zoom:number}|null>(null),[data,setData]=useState<Volume|null>(null),[bigBrain,setBigBrain]=useState<BigBrain|null>(null),[fixedBrain,setFixedBrain]=useState<FixedBrain|null>(null),[manualSeg,setManualSeg]=useState<ManualSeg|null>(null),[meshes,setMeshes]=useState<{surface:Mesh[];segments:Mesh[];overlays:Mesh[];basal:Mesh[];deep:Mesh[];landmarks:Mesh[]}|null>(null),[selectionLayers,setSelectionLayers]=useState<{meshes:Mesh[];color:[number,number,number]}[]>([]),[blockMeshes,setBlockMeshes]=useState<LoadedSpecimenPart[]|null>(null),[error,setError]=useState(""),[retryVersion,setRetryVersion]=useState(0),[sizeVersion,setSizeVersion]=useState(0),[zoom,setZoom]=useState(initialZoom.current),[pan,setPan]=useState({x:0,y:0});
  const quizNeurovascularOverlay:NeurovascularOverlay=surfaceHighlights.some(layer=>layer.ids.some(id=>id>=QUIZ_NERVE_ID_OFFSET))?"nerves":surfaceHighlights.some(layer=>layer.ids.some(id=>id>=QUIZ_VESSEL_ID_OFFSET))?"vessels":"none";
  const effectiveNeurovascularOverlay=neurovascularOverlay!=="none"?neurovascularOverlay:quizNeurovascularOverlay;
  const effectiveSurfaceView=quizNeurovascularOverlay!=="none"?"ghost":view,effectiveShowCerebellum=quizNeurovascularOverlay!=="none"?false:showCerebellum;
  const effectiveSurfaceHighlights=surfaceHighlights.map(layer=>({...layer,ids:layer.ids.filter(id=>id<QUIZ_VESSEL_ID_OFFSET)})).filter(layer=>layer.ids.length>0);
  const decodedQuizHighlights=surfaceHighlights.map(layer=>({...layer,ids:layer.ids.filter(id=>id>=QUIZ_VESSEL_ID_OFFSET).map(id=>id>=QUIZ_NERVE_ID_OFFSET?id-QUIZ_NERVE_ID_OFFSET:id-QUIZ_VESSEL_ID_OFFSET)})).filter(layer=>layer.ids.length>0);
  const effectiveNeurovascularHighlights=[...neurovascularHighlights,...decodedQuizHighlights];
  const surfaceLandmarkKey=surfaceLandmarks.join(","),surfaceDeepLandmarkKey=surfaceDeepLandmarks.join(","),surfaceHighlightKey=surfaceHighlights.flatMap(layer=>layer.ids).join(",");
  ...
  useEffect(()=>{
    ...
    const segmented=effectiveSurfaceView==="segmented",wantVessels=effectiveNeurovascularOverlay==="vessels"||effectiveNeurovascularOverlay==="both",wantNerves=effectiveNeurovascularOverlay==="nerves"||effectiveNeurovascularOverlay==="both";
    ...
    return()=>{active=false};
  },[kind,specimenBlock,effectiveSurfaceView,effectiveNeurovascularOverlay,showBasalLandmarks,surfaceLandmarkKey,surfaceDeepLandmarkKey,surfaceHighlightKey,retryVersion]);
  useEffect(()=>{
  ...
  useEffect(()=>{const el=ref.current;if(!el)return;const dpr=Math.min(devicePixelRatio||1,2),w=el.clientWidth,h=el.clientHeight;if(w<1||h<1)return;el.width=w*dpr;el.height=h*dpr;const labelColors=new Map<number,[number,number,number]>();highlights.forEach(layer=>layer.ids.forEach(id=>labelColors.set(id,layer.color)));if(kind==="slice"&&contrast==="single"&&fixedBrain){const c=el.getContext("2d");if(c){c.setTransform(dpr,0,0,dpr,0,0);drawFixedSlice(c,w,h,fixedBrain,plane,position,display,DISPLAY_TONE,zoom,pan)}}else if(kind==="slice"&&contrast==="bigbrain"&&bigBrain){const c=el.getContext("2d");if(c){c.setTransform(dpr,0,0,dpr,0,0);drawSlice(c,w,h,null,bigBrain,manualSeg,plane,position,display,"bigbrain",DISPLAY_TONE,labelColors,zoom,pan)}}else if(kind==="slice"&&data){const c=el.getContext("2d");if(c){c.setTransform(dpr,0,0,dpr,0,0);drawSlice(c,w,h,data,null,null,plane,position,display,contrast==="t2"?"t2":"t1",DISPLAY_TONE,labelColors,zoom,pan)}}else if(kind==="surface"&&meshes&&(specimenBlock==="none"||blockMeshes))drawWebGL(el,selectionLayers,meshes.surface,meshes.segments,meshes.overlays,meshes.basal,meshes.deep,meshes.landmarks,blockMeshes,rotation,plane,position,effectiveSurfaceView,showFocus,contrast,showCutPlane,hemisphere,effectiveShowCerebellum,showPonsMedulla,showMidbrain,effectiveNeurovascularOverlay,showBrainstemNerves,effectiveSurfaceHighlights,surfaceLandmarks,surfaceDeepLandmarks,effectiveNeurovascularHighlights,specimenLayers,specimenTissueMode,showBasalLandmarks,basalLandmark,basalHighlights,basalOnlySelected,specimenBlock,zoom);},[data,bigBrain,fixedBrain,manualSeg,meshes,selectionLayers,blockMeshes,kind,plane,position,focus,display,rotation,effectiveSurfaceView,contrast,sizeVersion,highlights,surfaceHighlights,surfaceLandmarks,surfaceDeepLandmarks,neurovascularHighlights,specimenLayers,specimenTissueMode,showFocus,showCutPlane,hemisphere,effectiveShowCerebellum,showPonsMedulla,showMidbrain,specimenBlock,effectiveNeurovascularOverlay,showBrainstemNerves,showBasalLandmarks,basalLandmark,basalHighlights,basalOnlySelected,zoom,pan]);
  ...
  function pointerDown(e:React.PointerEvent<HTMLCanvasElement>){if(kind==="surface"&&onSurfaceIdentify)surfaceClick.current={x:e.clientX,y:e.clientY,moved:false};if(kind==="slice"&&(e.button===1||e.shiftKey)){e.currentTarget.setPointerCapture(e.pointerId);panDrag.current={x:e.clientX,y:e.clientY,pan};return}if(kind==="slice"&&e.pointerType==="touch"){sliceTouchClick.current={x:e.clientX,y:e.clientY,moved:false};return}identify(e)}
  function pointerMove(e:React.PointerEvent<HTMLCanvasElement>){if(surfaceClick.current&&Math.hypot(e.clientX-surfaceClick.current.x,e.clientY-surfaceClick.current.y)>5)surfaceClick.current.moved=true;if(sliceTouchClick.current&&Math.hypot(e.clientX-sliceTouchClick.current.x,e.clientY-sliceTouchClick.current.y)>5)sliceTouchClick.current.moved=true;const drag=panDrag.current;if(!drag)return;setPan({x:drag.pan.x+e.clientX-drag.x,y:drag.pan.y+e.clientY-drag.y});onViewChange?.()}
  function pointerUp(e:React.PointerEvent<HTMLCanvasElement>){if(surfaceClick.current){const click=surfaceClick.current;surfaceClick.current=null;if(!click.moved)identifySurface(e)}if(sliceTouchClick.current){const click=sliceTouchClick.current;sliceTouchClick.current=null;if(!click.moved)identify(e)}if(panDrag.current)panDrag.current=null;if(e.currentTarget.hasPointerCapture(e.pointerId))e.currentTarget.releasePointerCapture(e.pointerId)}
  function pointerCancel(){surfaceClick.current=null;sliceTouchClick.current=null;panDrag.current=null}
  function pinchDistance(touches:React.TouchList){const a=touches.item(0),b=touches.item(1);return a&&b?Math.hypot(a.clientX-b.clientX,a.clientY-b.clientY):0}
  function pinchStart(e:React.TouchEvent<HTMLCanvasElement>){if(kind!=="surface"||e.touches.length!==2)return;const distance=pinchDistance(e.touches);if(distance<1)return;pinch.current={distance,zoom};e.preventDefault();e.stopPropagation();onViewChange?.()}
  function pinchMove(e:React.TouchEvent<HTMLCanvasElement>){if(kind!=="surface"||e.touches.length!==2||!pinch.current)return;const distance=pinchDistance(e.touches);if(distance<1)return;e.preventDefault();e.stopPropagation();setZoom(Math.max(.7,Math.min(2.4,pinch.current.zoom*distance/pinch.current.distance)));onViewChange?.()}
  function pinchEnd(e:React.TouchEvent<HTMLCanvasElement>){if(e.touches.length<2)pinch.current=null}
  function resetView(){setZoom(initialZoom.current);setPan({x:0,y:0});onViewChange?.()}
  ...
  const ready=kind==="slice"?(contrast==="single"?!!fixedBrain:contrast==="bigbrain"?!!bigBrain&&!!manualSeg:!!data):!!meshes&&(specimenBlock==="none"||!!blockMeshes);return <><canvas ref={ref} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerCancel} onTouchStart={pinchStart} onTouchMove={pinchMove} onTouchEnd={pinchEnd} onTouchCancel={pinchEnd} onWheel={handleWheel} onDoubleClick={resetView} className={`atlasCanvas ${(kind==="slice"&&onIdentify)||(kind==="surface"&&onSurfaceIdentify)?"identifiable":""} ${zoom>initialZoom.current*1.01?"zoomed":""}`} aria-label={kind==="surface"?`${specimenBlock==="none"?"MNI高密度皮質表面モデル":"0.5 mm標本から構成した局所3D標本"}${neurovascularOverlay!=="none"?"と模式3D神経血管レイヤー":""}。ホイールまたは2本指で拡大縮小${showZoomControls?"、画面ボタンでも操作可能":""}${onSurfaceIdentify?"、クリックで構造を選択":""}`:`${plane}断面 ${position}。ホイールで拡大縮小、Shiftドラッグで移動`}/>{kind==="surface"&&showZoomControls?<div className="modelZoomControls" aria-label="3D表示の拡大縮小" onPointerDown={event=>event.stopPropagation()}><button type="button" onClick={()=>adjustSurfaceZoom(1/1.15)} aria-label="縮小">−</button><button type="button" onClick={resetView} aria-label="表示倍率を100パーセントに戻す" title="100%に戻す">{Math.round(zoom/initialZoom.current*100)}%</button><button type="button" onClick={()=>adjustSurfaceZoom(1.15)} aria-label="拡大">＋</button></div>:kind==="slice"&&Math.abs(zoom-1)>.01&&<button type="button" className="zoomReadout" onClick={resetView} title="表示を等倍に戻す">{Math.round(zoom*100)}% <small>リセット</small></button>}{(!ready||error)&&(error?<div className="atlasLoading error" role="alert"><b>データを読み込めませんでした</b><button type="button" onClick={retryLoad}>再読み込み</button></div>:<span className="atlasLoading" role="status"><span>{specimenBlock!=="none"?"局所標本を読み込み中…":contrast==="single"?"0.444 mm 単一固定脳を読み込み中…":contrast==="bigbrain"?"組織切片データを読み込み中…":"1 mm 解剖データを読み込み中…"}</span><progress aria-label="データ読込の進捗" /></span>)}</>;
}
...
function drawWebGL(canvas:HTMLCanvasElement,selectionLayers:{meshes:Mesh[];color:[number,number,number]}[],surface:Mesh[],segments:Mesh[],overlays:Mesh[],basal:Mesh[],deep:Mesh[],landmarks:Mesh[],blockMeshes:LoadedSpecimenPart[]|null,rot:Rotation,plane:Plane,position:number,view:"inside"|"ghost"|"extracted"|"segmented",showFocus:boolean,contrast:"t1"|"t2"|"bigbrain"|"single",showCutPlane:boolean,hemisphere:"both"|"left"|"right",showCerebellum:boolean,showPonsMedulla:boolean,showMidbrain:boolean,neurovascularOverlay:NeurovascularOverlay,showBrainstemNerves:boolean,surfaceHighlights:HighlightLayer[],surfaceLandmarks:SurfaceLandmark[],surfaceDeepLandmarks:SurfaceDeepLandmark[],neurovascularHighlights:HighlightLayer[],specimenLayers:string[],specimenTissueMode:SpecimenTissueMode,showBasalLandmarks:boolean,basalLandmark:BasalLandmark,basalHighlights:BasalLandmark[],basalOnlySelected:boolean,specimenBlock:SpecimenBlock,zoom:number){
  ...
  const target=targetCanvas.getContext("2d");if(target){target.clearRect(0,0,targetCanvas.width,targetCanvas.height);target.drawImage(canvas,0,0)}
}
