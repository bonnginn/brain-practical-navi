...
const SURFACE_DEEP_LANDMARKS:{key:SurfaceDeepLandmark;color:[number,number,number,number]}[]=[
  ...
  {key:"septum-pellucidum",color:[.66,.77,.74,1]},
...
function drawWebGL(canvas:HTMLCanvasElement,brain:Mesh,focus:Mesh,surface:Mesh[],segments:Mesh[],overlays:Mesh[],basal:Mesh[],deep:Mesh[],landmarks:Mesh[],blockMeshes:LoadedSpecimenPart[]|null,rot:Rotation,focusColor:[number,number,number],plane:Plane,position:number,view:"inside"|"ghost"|"extracted"|"segmented",showFocus:boolean,contrast:"t1"|"t2"|"bigbrain"|"single",showCutPlane:boolean,hemisphere:"both"|"left"|"right",showCerebellum:boolean,showPonsMedulla:boolean,neurovascularOverlay:NeurovascularOverlay,surfaceHighlights:HighlightLayer[],surfaceLandmarks:SurfaceLandmark[],surfaceDeepLandmarks:SurfaceDeepLandmark[],neurovascularHighlights:HighlightLayer[],specimenLayers:string[],specimenTissueMode:SpecimenTissueMode,showBasalLandmarks:boolean,basalLandmark:BasalLandmark,basalHighlights:BasalLandmark[],basalOnlySelected:boolean,zoom:number){
  ...
  if(surfaceLandmarks.length&&blockMeshes===null){
    ...
    SURFACE_LANDMARKS.forEach((definition,index)=>{if(!surfaceLandmarks.includes(definition.key))return;if(definition.key==="longitudinal-fissure")draw(landmarks[index],definition.color,1);else if(definition.key==="calcarine-sulcus")visibleSurface.slice(0,2).forEach(part=>draw(surfaceLevelMesh(part,[57,6],0,-14,.9),definition.color,3));else visibleSurface.slice(0,2).forEach(part=>draw(surfaceBoundaryMesh(part,definition.key,2.05,.9),definition.color,3))});
    gl.uniform1f(gl.getUniformLocation(prog,"hemiMode"),0);
  }
  if(surfaceDeepLandmarks.length&&blockMeshes===null){
    if(hemisphere!=="both")gl.clear(gl.DEPTH_BUFFER_BIT);gl.uniform1f(gl.getUniformLocation(prog,"clipOn"),0);gl.uniform1f(gl.getUniformLocation(prog,"hemiMode"),hemisphere==="left"?-1:hemisphere==="right"?1:0);gl.disable(gl.CULL_FACE);gl.depthFunc(gl.LEQUAL);
    SURFACE_DEEP_LANDMARKS.forEach((definition,index)=>{if(surfaceDeepLandmarks.includes(definition.key))draw(deep[index],definition.color,1)});
    gl.uniform1f(gl.getUniformLocation(prog,"hemiMode"),0);
  }
