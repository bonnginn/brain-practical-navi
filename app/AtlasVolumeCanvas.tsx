...
type BasalLandmark="all"|"olfactory"|"optic"|"infundibulum"|"mammillary"|"perforated"|"peduncles"|"pyramids"|"olives"|"hypothalamic";
...
const SURFACE_BOUNDARY_LABELS:Partial<Record<SurfaceLandmark,{a:number[];b:number[]}>>={
  ...
  "lateral-sulcus":{a:[96,45],b:[83,32,73,22,86,35,64,13,102,51]},
...
export function AtlasVolumeCanvas({kind,plane,position,focus,display,rotation,view="inside",contrast="t1",highlights=[],surfaceHighlights=[],surfaceLandmarks=[],surfaceDeepLandmarks=[],neurovascularHighlights=[],specimenLayers=[],specimenTissueMode="solid",onIdentify,onSurfaceIdentify,onViewChange,showFocus=true,showCutPlane=true,hemisphere="both",showCerebellum=true,showPonsMedulla=true,showMidbrain=true,specimenBlock="none",neurovascularOverlay="none",showBasalLandmarks=false,basalLandmark="all",basalHighlights=[],basalOnlySelected=false}:{kind:"surface"|"slice";plane:Plane;position:number;focus:Focus;display:Display;rotation:Rotation;view?:"inside"|"ghost"|"extracted"|"segmented";contrast?:"t1"|"t2"|"bigbrain"|"single";highlights?:HighlightLayer[];surfaceHighlights?:HighlightLayer[];surfaceLandmarks?:SurfaceLandmark[];surfaceDeepLandmarks?:SurfaceDeepLandmark[];neurovascularHighlights?:HighlightLayer[];specimenLayers?:string[];specimenTissueMode?:SpecimenTissueMode;onIdentify?:(point:IdentifiedPoint)=>void;onSurfaceIdentify?:(point:SurfaceIdentifiedPoint)=>void;onViewChange?:()=>void;showFocus?:boolean;showCutPlane?:boolean;hemisphere?:"both"|"left"|"right";showCerebellum?:boolean;showPonsMedulla?:boolean;showMidbrain?:boolean;specimenBlock?:SpecimenBlock;neurovascularOverlay?:NeurovascularOverlay;showBasalLandmarks?:boolean;basalLandmark?:BasalLandmark;basalHighlights?:BasalLandmark[];basalOnlySelected?:boolean}){
  const ref=useRef<HTMLCanvasElement>(null),panDrag=useRef<{x:number;y:number;pan:{x:number;y:number}}|null>(null),surfaceClick=useRef<{x:number;y:number;moved:boolean}|null>(null),[data,setData]=useState<Volume|null>(null),[bigBrain,setBigBrain]=useState<BigBrain|null>(null),[fixedBrain,setFixedBrain]=useState<FixedBrain|null>(null),[manualSeg,setManualSeg]=useState<ManualSeg|null>(null),[meshes,setMeshes]=useState<{focus:Mesh;surface:Mesh[];segments:Mesh[];overlays:Mesh[];basal:Mesh[];deep:Mesh[];landmarks:Mesh[]}|null>(null),[blockMeshes,setBlockMeshes]=useState<LoadedSpecimenPart[]|null>(null),[error,setError]=useState(""),[sizeVersion,setSizeVersion]=useState(0),[zoom,setZoom]=useState(1),[pan,setPan]=useState({x:0,y:0});
  useEffect(()=>{if(kind==="slice"&&(contrast==="t1"||contrast==="t2"))loadVolume().then(setData).catch(e=>setError(String(e)))},[kind,contrast]);useEffect(()=>{if(kind!=="surface")return;if(specimenBlock!=="none"){setMeshes({focus:EMPTY_MESH,surface:[],segments:[],overlays:[],basal:[],deep:[],landmarks:[]});return}Promise.all([focus,"pial-left","pial-right","segment-cerebellum","segment-pons-medulla","segment-midbrain","segment-deep","segment-ventricles","overlay-arteries-anterior","overlay-arteries-posterior","overlay-nerves-anterior","overlay-nerves-pontine","overlay-nerves-medullary","landmark-olfactory-pathway","landmark-optic-pathway","landmark-infundibulum","landmark-mammillary-bodies","landmark-anterior-perforated-substance","block-midbrain-section-cerebral-peduncles","block-hindbrain-pyramids","block-hindbrain-olives","block-commissural-system-corpus-callosum","block-commissural-system-septum-pellucidum","block-commissural-system-fornix","block-diencephalon-thalami","block-diencephalon-hypothalamus",...SURFACE_LANDMARKS.map(item=>`surface-landmark-${item.key}`)].map(loadMesh)).then(([f,left,right,cerebellum,ponsMedulla,midbrain,deep,ventricles,...rest])=>setMeshes({focus:f,surface:[left,right,cerebellum,ponsMedulla,midbrain],segments:[left,right,cerebellum,ponsMedulla,midbrain,deep,ventricles],overlays:rest.slice(0,5),basal:rest.slice(5,13),deep:rest.slice(13,18),landmarks:rest.slice(18)})).catch(e=>setError(String(e)))},[kind,focus,specimenBlock]);
  useEffect(()=>{
...
function drawWebGL(canvas:HTMLCanvasElement,focus:Mesh,surface:Mesh[],segments:Mesh[],overlays:Mesh[],basal:Mesh[],deep:Mesh[],landmarks:Mesh[],blockMeshes:LoadedSpecimenPart[]|null,rot:Rotation,focusColor:[number,number,number],plane:Plane,position:number,view:"inside"|"ghost"|"extracted"|"segmented",showFocus:boolean,contrast:"t1"|"t2"|"bigbrain"|"single",showCutPlane:boolean,hemisphere:"both"|"left"|"right",showCerebellum:boolean,showPonsMedulla:boolean,showMidbrain:boolean,neurovascularOverlay:NeurovascularOverlay,surfaceHighlights:HighlightLayer[],surfaceLandmarks:SurfaceLandmark[],surfaceDeepLandmarks:SurfaceDeepLandmark[],neurovascularHighlights:HighlightLayer[],specimenLayers:string[],specimenTissueMode:SpecimenTissueMode,showBasalLandmarks:boolean,basalLandmark:BasalLandmark,basalHighlights:BasalLandmark[],basalOnlySelected:boolean,specimenBlock:SpecimenBlock,zoom:number){
  ...
  if(surfaceLandmarks.length&&blockMeshes===null){
    ...
    SURFACE_LANDMARKS.forEach((definition,index)=>{if(!surfaceLandmarks.includes(definition.key))return;if(definition.key==="longitudinal-fissure"||definition.key==="central-sulcus")draw(landmarks[index],definition.color,1);else if(definition.key==="calcarine-sulcus")visibleSurface.slice(0,2).forEach(part=>draw(surfaceLevelMesh(part,[57,6],0,-14,.9),definition.color,3));else visibleSurface.slice(0,2).forEach(part=>draw(surfaceBoundaryMesh(part,definition.key,2.05,.9),definition.color,3))});
  ...
  if(showBasalLandmarks){
    if(view==="ghost")gl.clear(gl.DEPTH_BUFFER_BIT);gl.uniform1f(gl.getUniformLocation(prog,"clipOn"),0);gl.disable(gl.CULL_FACE);const keys:BasalLandmark[]=["olfactory","optic","infundibulum","mammillary","perforated","peduncles","pyramids","olives"],palette=[[.86,.70,.43,1],[.88,.82,.67,1],[.78,.52,.46,1],[.70,.55,.43,1],[.66,.46,.38,1],[.82,.60,.33,1],[.82,.66,.39,1],[.74,.43,.36,1]],neutral=[.78,.82,.83,1],hypothalamicOnly=basalLandmark==="hypothalamic";basal.forEach((part,index)=>{if(hypothalamicOnly&&!(["infundibulum","mammillary"] as BasalLandmark[]).includes(keys[index]))return;const key=keys[index],active=hypothalamicOnly||basalHighlights.includes(key);if(basalOnlySelected&&!active)return;if((key==="pyramids"||key==="olives")&&!hypothalamicOnly){if(showPonsMedulla)draw(ventralSurfacePatchMesh(surface[3],key),active?[...palette[index]]:[...neutral],3);return}draw(part,active?[...palette[index]]:[...neutral],active?1:0)});
  }
