...
function makePatch(edits:Map<number,number>,note:string,authorGitHub:string):SegmentationPatch{return{format:"brain-practical-segmentation-patch",version:1,sourceImage:IMAGE_URL,sourceLabels:LABEL_URL,sourceLabelsSha256:LABEL_SHA256,dims:[394,466,378],voxelSizeMm:[.5,.5,.5],primaryPlane:"horizontal",createdAt:new Date().toISOString(),authorNote:note.trim(),authorGitHub:authorGitHub.trim().replace(/^@/,""),editCount:edits.size,runs:toRuns(edits)}}
...
export function ManualSegmentationWorkbench(){
  ...
  const[data,setData]=useState<VolumeData|null>(null),[error,setError]=useState(""),[position,setPosition]=useState(52),[label,setLabel]=useState(7),[tool,setTool]=useState<Tool>("paint"),[brush,setBrush]=useState(5),[showOriginal,setShowOriginal]=useState(true),[zoom,setZoom]=useState(1),[pan,setPan]=useState({x:0,y:0}),[version,setVersion]=useState(0),[undo,setUndo]=useState<StrokeChange[][]>([]),[redo,setRedo]=useState<StrokeChange[][]>([]),[note,setNote]=useState(""),[authorGitHub,setAuthorGitHub]=useState(""),[status,setStatus]=useState("端末内に未保存"),[sizeVersion,setSizeVersion]=useState(0),[cursor,setCursor]=useState<{x:number;y:number}|null>(null);
  ...
  function pointerDown(event:React.PointerEvent<HTMLCanvasElement>){if(event.button===1||event.button===2||event.altKey||event.metaKey){event.preventDefault();event.currentTarget.setPointerCapture(event.pointerId);panRef.current={x:event.clientX,y:event.clientY,pan};return}event.currentTarget.setPointerCapture(event.pointerId);strokeRef.current=[];strokeSeen.current=new Set;paintAt(event)}
  ...
  return <div className="segWorkbench">
    ...
    <aside className="segControls"><span className="guideIndex">CONTRIBUTOR TOOL · LOCAL DRAFT</span><h2>手動セグメンテーション</h2><p>ブラウザ上では元ラベルを変更せず、修正差分だけを端末に保存します。レビュー用JSONを書き出してPull Requestへ添付してください。</p>
      ...
      <div className="segFiles"><button className="primary" onClick={download} disabled={!editsRef.current.size}>差分JSONを書き出す</button><button onClick={()=>fileRef.current?.click()}>差分JSONを読み込む</button><input ref={fileRef} hidden type="file" accept="application/json,.json" onChange={event=>{const file=event.target.files?.[0];if(file)void importFile(file);event.currentTarget.value=""}}/><button className="danger" onClick={clear} disabled={!editsRef.current.size}>編集をすべて消去</button></div>
