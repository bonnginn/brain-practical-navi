...
import { OPTIC_REVIEW_CANDIDATES, type OpticReviewCandidate } from "./opticReviewCandidates";
...
const labelGroups=[
  ...
  {name:"部分分節のレビュー用",items:[[41,"中脳水道候補（部分）"]] as [number,string][]},
];
...
export function ManualSegmentationWorkbench(){
  ...
  function jumpToOpticReviewCandidate(candidate:OpticReviewCandidate){if(!data)return;setPlane(candidate.plane);setPosition(planePositionForSlice(candidate.sliceIndex,candidate.plane,data.dims));setZoom(1);setPan({x:0,y:0});setCursor(null);setCursorVoxel(null)}
  ...
  return <div className="segWorkbench">
    ...
    <aside className="segControls"><span className="guideIndex">CONTRIBUTOR TOOL · LOCAL DRAFT</span><h2>手動セグメンテーション</h2><p>ブラウザ上では元ラベルを変更せず、修正差分だけを端末に保存します。レビュー用JSONを書き出してPull Requestへ添付してください。</p>
      ...
      <div className="segOpticReviewPanel"><header><span>CONTRIBUTOR ONLY</span><b>旧ID33 データ順位候補</b></header><p>データ順位で選んだ出発点です。ボタンで表示位置へ移動しますが、編集は作成されません。解剖学的検証・専門家確認ではなく、ID 33を36–38へ機械的に分割する根拠でもありません。</p><div className="segOpticReviewButtons" role="group" aria-label="旧ID33 データ順位候補">{OPTIC_REVIEW_CANDIDATES.map(candidate=>{const active=plane===candidate.plane&&sliceIndex===candidate.sliceIndex;return <button key={candidate.id} type="button" onClick={()=>jumpToOpticReviewCandidate(candidate)} disabled={!data} aria-label={candidate.accessibleName} aria-pressed={active} className={active?"active":""}><b>{candidate.axis.toUpperCase()}{candidate.sliceIndex}</b><small>{candidate.voxelCountOnSlice.toLocaleString()} voxel</small></button>})}</div></div>
