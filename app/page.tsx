...
function overlayFromHash(hash:string):OverlayMode|null{const candidate=hash.replace(/^#/,"").replace(/^workspace\/?/,"").split("/")[0];return candidate==="help"||candidate==="feedback"||candidate==="legal"?candidate:null}
...
export default function Home() {
  ...
  const overlayOpen=helpOpen||feedbackOpen||legalOpen;
  ...
  return <main className={`appShell workspace-${workspace} ${workspace==="home"?"homeShell":""}`}>
    ...
    <aside id="mobile-context-panel" className={`leftRail rail-${workspace} ${mobileRailOpen?"mobileOpen":""}`} role={compactViewport?"dialog":undefined} aria-modal={compactViewport&&mobileRailOpen?true:undefined} aria-hidden={compactViewport&&!mobileRailOpen?true:undefined} aria-label={compactViewport?"この画面の設定":undefined} key={`rail-${workspace}`}>
