      ...
      <div className="visualGrid"><section className="slicePanel">
        ...
        <div className={`sliceStage ${plane} ${sliceVariant(position)} layout-${sectionLayout}`}>
          ...
          {sectionLayout!=="slice"&&<aside className="modelInset" aria-label={phoneViewport?"全脳モデルで切断位置を確認":"全脳を2方向から見て切断位置を確認"}>
            ...
            <div className="insetViews">{sectionModelRotations.slice(0,phoneViewport?1:2).map((modelRotation,index)=><div key={index} className="modelStage insetStage" tabIndex={0} aria-label={`切断位置の全脳3Dモデル・${index===0?"基準方向":"直交方向"}。ドラッグまたは矢印キーで回転、Rキーで向きを戻す`} onKeyDown={handleModelKey} onPointerDown={beginRotation} onPointerMove={move} onPointerUp={()=>setDrag(null)} onPointerCancel={()=>setDrag(null)} onContextMenu={event=>event.preventDefault()}>
