...
export default function Home() {
  ...
  useEffect(()=>{const close=(event:KeyboardEvent)=>{if(event.key==="Escape"){closeOverlay();setDetailsOpen(false);setMobileRailOpen(false)}};window.addEventListener("keydown",close);return()=>window.removeEventListener("keydown",close)},[workspace,surfaceView,plane,blockSpecimen]);
  ...
  return <main className={`appShell workspace-${workspace} ${workspace==="home"?"homeShell":""}`}>
    ...
    {workspace!=="home"&&<><button className={`mobileRailBackdrop ${mobileRailOpen?"visible":""}`} aria-label="画面設定を閉じる" onClick={()=>setMobileRailOpen(false)}/><button className="mobileContextToggle" aria-expanded={mobileRailOpen} aria-controls="mobile-context-panel" onClick={()=>setMobileRailOpen(value=>!value)}><span>{workspace==="sections"?"断面・構造":workspace==="surface"?"観察方向":workspace==="blocks"?"標本を選ぶ":workspace==="quiz"?"出題設定":"編集手順"}</span><b>{mobileRailOpen?"閉じる":"開く"}</b></button></>}
    <aside id="mobile-context-panel" className={`leftRail rail-${workspace} ${mobileRailOpen?"mobileOpen":""}`} key={`rail-${workspace}`}>
      <div className="mobileRailSheetHead"><b>{workspace==="sections"?"断面と表示構造":workspace==="surface"?"観察方向":workspace==="blocks"?"局所標本":workspace==="quiz"?"復習クイズの設定":"編集ツールの手順"}</b><button aria-label="画面設定を閉じる" onClick={()=>setMobileRailOpen(false)}>×</button></div>
      {workspace==="sections"&&<>
