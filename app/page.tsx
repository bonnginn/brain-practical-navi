...
type QuizConceptFields = { id?:string; questionKind?:QuizQuestionKind; correctAnswer?:string; optionLabels?:Record<string,string>; explanation?:string };
...
const neurovascularStructures:Record<NeurovascularStructureKey,{name:string;latin:string;kind:"arteries"|"nerves";ids:number[];note:string}>={
  ...
  cn5:{name:"V 三叉神経",latin:"Trigeminal nerve",kind:"nerves",ids:[30,31],note:"配置を修正中です。現行の模式管が側頭葉組織へ入り込む不一致を確認したため、正しい経路の同定図として使わないでください。関連クイズは保留しています。感覚根・運動根も描き分けていません。"},
  ...
  cn9:{name:"IX 舌咽神経",latin:"Glossopharyngeal nerve",kind:"nerves",ids:[38,39],note:"配置を修正中です。現行の模式管が小脳組織へ入り込む不一致を確認したため、正しい経路の同定図として使わないでください。関連クイズは保留しています。複数の根糸と集合過程も未再現です。"},
  cn10:{name:"X 迷走神経",latin:"Vagus nerve",kind:"nerves",ids:[40,41],note:"配置を修正中です。現行の模式管が小脳組織へ入り込む不一致を確認したため、正しい経路の同定図として使わないでください。関連クイズは保留しています。複数の根糸も未再現です。"},
  cn11:{name:"XI 副神経",latin:"Accessory nerve",kind:"nerves",ids:[42,43],note:"配置を修正中です。現行の模式管が小脳組織へ入り込む不一致を確認したため、正しい経路の同定図として使わないでください。関連クイズは保留しています。根列や脊髄根の上行経路も未再現で、副神経全体の同定図としては使えません。"},
...
function freeObservationAtlasNomenclatureLabel(item:FreeObservationItem){return item.key.startsWith("region:")?surfaceAtlasNomenclatureLabel(item.key.slice("region:".length)):null}
...
const neurovascularQuizQuestions:NeurovascularQuizQuestion[]=[
  ...
  {target:"acomm",category:"neurovascular",view:"arteries",format:"neurovascular",detail:"arteries",origin:"provisional",prompt:"白色で強調された模式3Dの名称はどれですか？",options:["acomm","aca","pcomm","basilar"]},
  ...
  {target:"pcomm",category:"neurovascular",view:"arteries",format:"neurovascular",detail:"arteries",origin:"provisional",prompt:"白色で強調された模式3Dの名称はどれですか？",options:["pcomm","acomm","pca","ica"]},
  ...
  {target:"cerebellarArteries",category:"neurovascular",view:"arteries",format:"neurovascular",detail:"arteries",origin:"provisional",prompt:"白色で強調された模式3Dの名称はどれですか？",options:["cerebellarArteries","vertebral","basilar","pca"]},
  ...
  {target:"opticChiasm",category:"neurovascular",view:"cranialNerves",format:"neurovascular",detail:"cranialNerves",origin:"provisional",prompt:"白色で強調された模式3Dの名称はどれですか？",options:["opticChiasm","cn2","cn1","cn3"]},
...
const conceptQuizQuestions:QuizQuestion[]=quizConceptData.questions.map(seed=>{
  ...
  return {...visual,id:seed.id,questionKind:seed.kind,prompt:seed.prompt,correctAnswer:seed.correctAnswer,options:seed.options.map(option=>option.key),optionLabels:Object.fromEntries(seed.options.map(option=>[option.key,option.label])),explanation:seed.explanation,origin:"provisional"};
});
...
useEffect(()=>{const restore=()=>{const overlay=overlayFromHash(window.location.hash);setHelpOpen(overlay==="help");setFeedbackOpen(overlay==="feedback");setLegalOpen(overlay==="legal");setStatusOpen(overlay==="status");setPhoneSettingsOpen(false);const requestedWorkspace=workspaceFromHash(window.location.hash);const nextWorkspace=publicWorkspaceForLocale(requestedWorkspace,locale) as WorkspaceMode;if(englishEdition&&nextWorkspace==="home"&&(requestedWorkspace==="collaborate"||requestedWorkspace==="segment"))window.history.replaceState(null,"",workspaceHash("home"));setModelStrategyComparisonOpen(nextWorkspace==="collaborate"&&modelStrategyFromHash(window.location.hash));transitionBlockContextState({type:"restore-route",workspace:nextWorkspace,specimen:blockSpecimenFromHash(window.location.hash)});setBlockContextDrag(null);setWorkspace(nextWorkspace);if(nextWorkspace==="surface")chooseSurface(surfaceViewFromHash(window.location.hash),"none");else if(nextWorkspace==="sections")restoreSectionRoute();else if(nextWorkspace==="blocks")chooseBlock(blockSpecimenFromHash(window.location.hash),"none")};restore();window.addEventListener("hashchange",restore);window.addEventListener("popstate",restore);return()=>{window.removeEventListener("hashchange",restore);window.removeEventListener("popstate",restore)}},[]);
  ...
  function restoreAllQuiz(){setQuizWrongOnly(false);setQuizKind("all");setQuizCategory("all");setQuizFormat("all");setQuizDetail("all");setQuizIncludeProvisional(true);setQuizQueue(shuffledQuestions(allQuizQuestions).slice(0,quizCount));resetQuiz()}
...
{(Object.keys(planeData) as Plane[]).map((p, i) => <button key={p} className={`planeBtn ${plane === p ? "active" : ""}`} aria-current={plane===p?"page":undefined} onClick={() => jump(p)}><span>0{i + 1}</span><b>{planeData[p].ja}</b><small>{planeData[p].en}</small></button>)}
      ...
      <div className="learningGrid">
        <section className="learningModelCard surfaceModelCard"><div className="panelHead"><div><b>{surfaceLesson.name}</b><small>{surfaceLesson.en}・ドラッグで回転</small></div><div className="panelActions">{surfaceView==="free"?<span>{freeSelections.length} 構造を選択中</span>:surfaceNeurovascular?<span>3D OVERLAY · PILOT</span>:surfaceView!=="medial"?<button className={surfaceCerebellum?"active":""} aria-pressed={surfaceCerebellum} onClick={()=>setSurfaceCerebellum(value=>!value)} disabled={webglUnavailable}>{surfaceCerebellum?"小脳を外す":"小脳を戻す"}</button>:null}<button onClick={resetSurfaceView} disabled={webglUnavailable}>向きを戻す</button></div></div>
          <div className={`learningModelStage modelStage ${webglUnavailable?"webglUnavailable":""}`} data-rotation-x={rotation.x} data-rotation-y={rotation.y} data-rotation-z={rotation.z??0} tabIndex={webglUnavailable?undefined:0} aria-label={webglUnavailable?undefined:"脳表3Dモデル。ドラッグまたは矢印キーで回転、Rキーで向きを戻す"} onKeyDown={webglUnavailable?undefined:handleModelKey} onPointerDown={webglUnavailable?undefined:beginRotation} onPointerMove={webglUnavailable?undefined:move} onPointerUp={webglUnavailable?undefined:()=>setDrag(null)} onPointerCancel={webglUnavailable?undefined:()=>setDrag(null)} onContextMenu={webglUnavailable?undefined:event=>event.preventDefault()}>
            ...
            {surfaceView==="free"&&<div className="freeSelectionOverlay" aria-live="polite"><small>クリック／検索で複数選択</small><b>{freeFocusedItem?.name??(freeSelectedItems.length?`${freeSelectedItems.length}構造を着色中`:"構造を選択してください")}</b>{freeFocusedItem&&<><span>{anatomyDisplayEnglish(freeFocusedItem.latin)}</span>{freeObservationAtlasNomenclatureLabel(freeFocusedItem)&&<em className="surfaceAtlasNomenclatureMarker">{freeObservationAtlasNomenclatureLabel(freeFocusedItem)}</em>}</>}</div>}
        ...
        <aside className="learningGuide" key={surfaceView}>
          ...
          {surfaceView==="free"?<div className="freeExplorer">
            ...
            {normalizedFreeSearch&&<div className="freeSearchResults" aria-label="検索結果"><div className="freeResultSummary"><b>{freeFilteredItems.length}件</b><span>クリックして表示へ追加</span></div>{freeFilteredItems.length?<div>{freeFilteredItems.map(item=>{const active=freeSelectedSet.has(item.key),atlasMarker=freeObservationAtlasNomenclatureLabel(item);return <button key={item.key} className={active?"active":""} aria-pressed={active} onClick={()=>selectFreeObservation(item.key)}><i style={{background:item.color}}/><span><b>{item.name}</b><small>{anatomyDisplayEnglish(item.latin)}</small>{atlasMarker&&<small className="surfaceAtlasNomenclatureMarker">{atlasMarker}</small>}</span><em>{item.kind}</em><strong>{active?"✓":"＋"}</strong></button>})}</div>:<p>該当する構造はありません。</p>}</div>}
            <label className="freeStructureIndex"><span><b>構造索引</b><small>分類から名称を選択</small></span><select value="" onChange={event=>{if(event.target.value)selectFreeObservation(event.target.value as FreeObservationKey)}}><option value="">構造を選んで追加…</option>{freeObservationKinds.map(kind=><optgroup key={kind} label={kind}>{freeObservationItems.filter(item=>item.kind===kind).map(item=>{const atlasMarker=freeObservationAtlasNomenclatureLabel(item),atlasCompactLabel=item.key.startsWith("region:")?surfaceAtlasNomenclatureCompactLabel(item.key.slice("region:".length)):null;return atlasCompactLabel?<option key={item.key} value={item.key}>{item.name} — {anatomyDisplayEnglish(item.latin)} {atlasCompactLabel}</option>:<option key={item.key} value={item.key}>{item.name} — {anatomyDisplayEnglish(item.latin)}</option>})}</optgroup>)}</select></label>
    ...
    {workspace==="blocks"&&blockIntroOpen&&<section className="workArea blockIntroPage" id="workspace" tabIndex={-1}>
      <div className="blockIntroCard"><span>PROTOTYPE</span><h1>ブロック標本は試作中です</h1><p>位置関係を検討するための試作品であり、形状・範囲・接続関係の完全性や解剖学的正確性は保証しません。</p><div><button type="button" data-block-intro-action="close" onClick={()=>setBlockIntroOpen(false)}>試作品を確認する</button><button type="button" data-block-intro-action="feedback" onClick={()=>openOverlay("feedback")}>誤りを報告する</button></div></div>
    </section>}
    ...
    {workspace==="blocks"&&!blockIntroOpen&&<section className={`workArea learningArea ${blockContextVisible?"blockContext-open":""}`} id="workspace" tabIndex={-1}>
      ...
      <div className={`learningGrid ${blockContextVisible?"blockContext-active":""}`}>
        <section className="learningModelCard blockModelCard"><div className="panelHead"><div><b>{specimenLesson.name}</b><small>{specimenLesson.en}・ドラッグ：回転／Shift・右：傾き</small></div><span>SPECIMEN + SELECTABLE PARTS</span></div>
    ...
    {workspace==="quiz"&&<section className="workArea quizArea" id="workspace" tabIndex={-1}>
      ...
      {quizFinished?<div className="quizEmptyState quizResultState" role="status" aria-live="polite"><span>QUIZ COMPLETE</span><h2>{quizScore} / {quizQueue.length} 問正解</h2><p>{quizScore===quizQueue.length?"全問正解です。別の項目へ進むか、同じ問題を順番を変えて再確認できます。":"間違えた問題は端末内に保存しました。下の構造から観察画面へ戻るか、左の「間違った問題のみ」から再出題できます。"}</p>{quizMisses.length>0&&<div className="quizReviewTargets" aria-label="今回間違えた構造">{quizMisses.map(target=>{const question=quizQueue.find(item=>item.target===target);if(!question)return null;const item=isNeurovascularQuiz(question)?neurovascularStructures[question.target]:isSurfaceQuiz(question)?surfaceRegions[question.target]:structures[question.target];return <button key={target} onClick={()=>reviewQuizQuestion(question)}><b>{item.name}</b><small>{isNeurovascularQuiz(question)||isSurfaceQuiz(question)?surfaceViews[question.view].name:`${planeData[question.plane].ja}・位置 ${question.position}`}</small><span>観察画面で位置を確認 →</span></button>})}</div>}<div><button onClick={retryQuiz}>同じ問題を再挑戦</button><button onClick={startQuiz}>この条件で新しく出題</button></div></div>:quizEmpty?<div className="quizEmptyState" role="status"><span>REVIEW CACHE</span><h2>{quizWrongOnly&&wrongTargets.length===0?"間違い履歴がありません":"今回の出題はありません"}</h2><p>{quizWrongOnly&&wrongTargets.length===0?"間違い履歴がまだありません。左の「次回出題条件」で「間違った問題のみ」を解除するか、通常の条件で出題してください。":"現在の問題キューは空です。左の「次回出題条件」と候補数を確認し、「この条件で出題」を押してください。フィルタ変更は現在の問題ではなく次回の出題に反映されます。"}</p><button onClick={restoreAllQuiz}>標準問題へ戻る</button></div>:<div className="quizWorkspace">
        ...
        <aside className="quizQuestionCard" data-quiz-target={quizQuestion.target} data-quiz-kind={quizQuestionKind} data-quiz-format={neurovascularQuiz?"neurovascular":surfaceQuiz?"surface":"section"} data-quiz-plane={quizModelQuestion?undefined:quizQuestion.plane} data-quiz-position={quizModelQuestion?undefined:quizQuestion.position} data-quiz-view={quizModelQuestion?quizQuestion.view:undefined} data-quiz-detail={quizQuestion.detail} data-quiz-queue-length={quizQueue.length} data-quiz-queue-index={quizIndex} data-quiz-inventory-audit={quizVisibilityAuditHighlight!==null?"quizVisibilityAudit":"off"} data-quiz-inventory-sha256={quizVisibilityAuditHighlight!==null?QUIZ_VISIBILITY_INVENTORY_SHA256:undefined}>
          ...
          {neurovascularQuiz?<span className="provisionalQuizFlag">模式3D・専門家未確認</span>:isProvisionalQuiz(quizQuestion)&&<span className="provisionalQuizFlag">試作・専門家未確認</span>}
