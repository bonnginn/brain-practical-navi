...
type OverlayMode = "help" | "offline" | "feedback" | "legal";
...
function overlayFromHash(hash:string):OverlayMode|null{const candidate=hash.replace(/^#/,"").replace(/^workspace\/?/,"").split("/")[0];return candidate==="help"||candidate==="offline"||candidate==="feedback"||candidate==="legal"?candidate:null}
...
export default function Home() {
  ...
  const overlayOpen=helpOpen||offlineOpen||feedbackOpen||legalOpen;
  ...
  function closeOverlay(){setHelpOpen(false);setOfflineOpen(false);setFeedbackOpen(false);setLegalOpen(false);const nextHash=workspaceHash(workspace,surfaceView,plane,blockSpecimen);if(window.location.hash!==nextHash)window.history.replaceState(null,"",nextHash)}
  function openOverlay(key:OverlayMode){overlayReturnFocus.current=document.activeElement instanceof HTMLElement?document.activeElement:null;window.history.pushState(null,"",`#workspace/${key}`);setHelpOpen(key==="help");setOfflineOpen(key==="offline");setFeedbackOpen(key==="feedback");setLegalOpen(key==="legal")}
  ...
  function openWorkspace(key:WorkspaceMode){setHelpOpen(false);setOfflineOpen(false);setFeedbackOpen(false);setLegalOpen(false);setMobileRailOpen(false);const nextHash=workspaceHash(key,surfaceView,plane,blockSpecimen);if(window.location.hash!==nextHash)window.history.pushState(null,"",nextHash);setWorkspace(key);if(key==="home")setRotation({...homeRotation});if(key==="sections")setRotation({x:-7,y:-18,z:0});if(key==="surface")setRotation(surfaceViews[surfaceView].rotation);if(key==="blocks"){setBlockIntroOpen(!comparisonRequested);setRotation({...blockInitialRotations[blockSpecimen]});setBlockViewPreset("initial")}}
