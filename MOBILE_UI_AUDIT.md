...
## 2026-09-05 クイズ幅別調整（未公開）

幅1007pxのin-app browserで設定railが160pxまで細くなり、不自然な多段折り返しとselect表示の不足を目視した。PCクイズのみrailを250–290pxに確保。見出しと余白はremを含むclampで調整し、選択肢本文は1remを維持。利用可能なquiz-area幅42rem以下で画像／問題を上下配置、quiz-image幅32rem以下で断面スライダー操作を複数行にする。phone設定sheetの構成は変更しない。長い強調ラベルと画像見出しも枠内で折り返す。

Chromeローカルpreview4344、日英×1366/1007/800/390px（高さ768px）の8条件で、ページ横overflow、問題／選択肢／断面操作／強調ラベル／画像見出しの横overflow、画面内ロードエラーが0。本文16px、1366/1007では左右、800/390では上下配置を実測。390は明示ui=phone・fine pointerによる表示確認で、物理タッチ検証ではない。根拠は `work/quiz-responsive/report.json` とPNG。en-1007とja-390の画像を目視。静的回帰テスト2件、型検査、本番ビルド成功。全問題・物理端末・200%文字拡大・公開サイトは今回未検証。
