...
後続では固定成分85の1,314 voxelについて全99占有／隣接断を点検し、既存3間隙291 voxelと合わせた1,605 voxelだけを開発ラベルから除外した（[採用記録](CALLOSUM_LOCAL_REPAIR.md)、現行5348…）。6,528 voxelを自動削除したわけではない。補助分類と原画像目視を併用したAI支援のプロジェクト採用で、専門家承認ではない。
