"""Pinned fifth-stage edits and atomic rejection, not anatomical ground truth."""
import hashlib
import sys
import unittest
...
class CallosalFollowupTests(unittest.TestCase):
    def setUp(self):
        self.source=b.ROOT/'tests/fixtures/bigbrain-practical-segmentation-pre-callosal-followup-5348.bin.gz'
        ...
        self.record=b.ROOT/'segmentation-patches/review/callosum-cortical-followup-project-review-2026-09-06.json'

    def test_exact_fifth_stage(self):
        audit=b.apply_approved_callosal_followup_patch(self.volume,self.record)
        ...
        self.assertEqual(len(changed),1596)
        self.assertEqual(hashlib.sha256(changed.tobytes()).hexdigest(),b.CALLOSUM_FOLLOWUP_INDICES_SHA256)
        self.assertTrue(np.all(before[changed]==30));self.assertTrue(np.all(after[changed]==0))
        self.assertEqual(hashlib.sha256(after.tobytes()).hexdigest(),'3c9d959acbdb67b7603ed7f2f105d7c333f0f89facc7e637f16b5fb740a16cd5')
        self.assertEqual(int(np.sum(after==30)),148179)
    ...
    def test_late_provenance_failure_is_atomic(self):
        with patch.object(Path,'relative_to',side_effect=ValueError('outside repository')):
            with self.assertRaisesRegex(ValueError,'outside repository'):
                b.apply_approved_callosal_followup_patch(self.volume,self.record)
        self.assertEqual(self.volume.tobytes(order='F'),self.before)

    def test_wrong_review_or_wrong_exact_set_is_atomic(self):
        record,edits,metadata=validate_patch(self.record,self.volume.shape,self.volume.size,bytearray(self.before),b.CALLOSUM_FOLLOWUP_SOURCE_SHA256)
        for scenario in ('review','set','value'):
            ...
            with self.subTest(scenario=scenario),patch.object(b,'validate_patch',return_value=(proposed,changed,metadata)):
                with self.assertRaises(ValueError):b.apply_approved_callosal_followup_patch(self.volume,self.record)
                self.assertEqual(self.volume.tobytes(order='F'),self.before)
    ...
    def test_already_applied_or_modified_baseline_is_rejected(self):
        for scenario in ('modified','already-applied'):
            ...
            else:b.apply_approved_callosal_followup_patch(self.volume,self.record)
