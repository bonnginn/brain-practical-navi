...
test('current callosal correction is exactly 1605 project-reviewed 30-to-0 edits',async()=>{
 ...
 const compressed=await readFile(new URL('public/atlas/bigbrain-practical-segmentation-icbm500.bin.gz',root));
 ...
 assert.equal(metadata.labelCounts['30'],149775);assert.equal(metadata.rawVoxelSha256,sha(current.subarray(10)));
...
test('callosal block mesh carries the corresponding actual geometry metadata',async()=>{
 ...
 assert.equal(sha(data),'ae2fd10bc23547c47dc12558acf6c8868da97de3dc369a2e665fa23cb2eb9504');
