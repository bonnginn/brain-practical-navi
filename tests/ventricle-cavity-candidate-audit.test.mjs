...
const labels = localPath("tests/fixtures/bigbrain-practical-segmentation-pre-ventricle-6744.bin.gz");
...
test("finds only the pinned orthogonally bracketed ventricle repair candidate", async () => {
  ...
  try {
    ...
    const publishedBefore = digest(await readFile(publishedLabels));
    ...
    for (const field of ["format","version","sourceImage","sourceLabels","sourceLabelsSha256","dims","voxelSizeMm","primaryPlane","workflowMetadataVersion","targetSide","confidence","targetStructures","sliceRanges","changeSummary","editCount","runs"])
      assert.deepEqual(generatedPatch[field], committedPatch[field], field);
    assert.equal(generatedPatch.review.decision, "unreviewed");
    assert.equal(committedPatch.review.decision, "approved");
    assert.equal(committedPatch.review.pullRequest.number, 14);
    ...
    const checked = spawnSync(python.command, [...python.prefix, validator, savedPatch, "--input", labels, "--check"], {
      cwd: localPath(""), encoding: "utf8", maxBuffer: 1024 * 1024,
    });
    assert.equal(checked.status, 0, checked.stderr);
    const validation = JSON.parse(checked.stdout);
    assert.equal(validation.workflowMetadataStatus, "strict");
    assert.equal(validation.reviewStatus, "approved");
    assert.equal(validation.changedVoxelCount, 33);
    assert.equal(digest(await readFile(labels)), before, "read-only audit must not modify the pre-adoption fixture");
    assert.equal(digest(await readFile(publishedLabels)), publishedBefore, "read-only audit must not modify the published label volume");
  } finally {
...
test("renders pinned local-only orthogonal evidence without changing published labels", async () => {
  ...
  try {
    ...
    assert.equal(manifest.status, "project-reviewed-source-evidence");
    ...
    assert.equal(digest(await readFile(publishedLabels)), publishedBefore);
  } finally {
    await rm(temporary, {recursive:true, force:true});
  }
});
