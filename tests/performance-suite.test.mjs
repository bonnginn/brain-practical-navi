...
test("performance suite defines the fixed 31-entry coverage without launching Chrome", () => {
  const matrix = buildPerformanceMatrix();
  assert.equal(matrix.length, 31);
  assert.equal(new Set(matrix.map(entry => entry.key)).size, 31);

  const full = matrix.filter(entry => entry.viewportId === "pc" || entry.viewportId === "tablet-landscape");
  ...
  assert.equal(definition.entryCount, 31);
...
test("performance suite allPassed aggregates every result and rejects missing or failed entries", () => {
  ...
  assert.equal(report.matrix.results.length, 31);
  assert.equal(report.matrix.definition.entryCount, 31);
