...
import {lateralCavity21Successor} from './helpers/residual-mesh-successor.mjs';
...
test('lateral cisternal-side exclusion is exactly 80 reversible ID24-to-zero voxels',async()=>{
 const bytes=await read('segmentation-patches/review/lateral-residual80-adoption-2026-09-07.json'),r=JSON.parse(bytes);
 const base=await read('tests/fixtures/bigbrain-practical-segmentation-pre-lateral-residual80-7d2b.bin.gz'),current=await read('tests/fixtures/bigbrain-practical-segmentation-pre-lateral-cavity21-a512.bin.gz');
 ...
 assert.equal(r.points.length,80);assert.deepEqual(before.subarray(0,10),after.subarray(0,10));
 ...
 const latest=JSON.parse(await read('segmentation-patches/review/lateral-cavity21-adoption-2026-09-07.json'));
 const currentRepair=await withRegionalBatches(JSON.parse(await read('segmentation-patches/review/lateral-crop34-adoption-2026-09-07.json')));assert.equal(currentRepair.beforeSha256,latest.afterSha256);
 assert.equal(latest.beforeSha256,r.afterSha256);assert.equal(meta.rawVoxelSha256,currentRepair.afterRawVoxelSha256);assert.equal(meta.lateralResidual80Audit.recordSha256,sha(bytes));assert.equal(meta.labelCounts['24'],79082);
 ...
 for(const p of changed){const next=await lateralCavity21Successor(p.file,p.afterSha256);assert.ok(next);assert.equal(sha(await read('public/atlas/'+p.file)),next.afterSha256);assert.equal(sha(await read('tests/fixtures/'+p.file.slice(0,-5)+'-pre-lateral-residual80.mesh')),p.beforeSha256);const item=manifest.specimens[p.block].find(q=>q.part===p.part);assert.equal(item.meshSha256,next.afterSha256);assert.equal(item.segmentationSourceSha256,next.segmentationSourceSha256);}
 for(const [name,info] of Object.entries(r.sectionMeshImpact.after.meshes)){assert.equal(latest.sectionMeshImpact.before.meshes[name].sha256,info.sha256);assert.equal(currentRepair.sectionMeshImpact.before.meshes[name].sha256,latest.sectionMeshImpact.after.meshes[name].sha256);assert.equal(sha(await read('public/atlas/'+name+'.mesh')),currentRepair.sectionMeshImpact.after.meshes[name].sha256);}
