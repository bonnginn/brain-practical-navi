...
test("freezes exact 17/6/17 inventory, options/render dependency, and 120-row matrix",()=>{
  ...
  const matrix=buildQuizTargetVisibilityMatrix();assert.equal(matrix.length,120);assert.equal(new Set(matrix.map(row=>row.key)).size,120);assert.deepEqual(QUIZ_TARGET_VISIBILITY_VIEWPORTS.at(-1),{id:"phone",label:"phone",width:390,height:768,dpr:1,deviceScaleFactor:1,mobile:true,isMobile:true,touch:true,hasTouch:true,coarse:true,pointer:"coarse"});
...
test("unit fixture is external-artifact evidence and is rejected by live mode",()=>{
  ...
  assert.deepEqual(runQuizTargetVisibilityBrowserAudit(),{status:"incomplete",provenance:"live-browser",reason:"live CDP coordinator is not implemented at this checkpoint",expectedMatrixCount:120});
});
...
test("CLI help succeeds but every non-help coordinator invocation is explicitly incomplete",()=>{
  const script=fileURLToPath(scriptPath),help=spawnSync(process.execPath,[script,"--help"],{encoding:"utf8"});assert.equal(help.status,0,help.stderr);assert.match(help.stdout,/Live collection is incomplete/);
  const run=spawnSync(process.execPath,[script],{encoding:"utf8"});assert.notEqual(run.status,0);assert.match(run.stdout,/"status": "incomplete"/);
});
...
test("rejects fabricated provenance, matrix, capture identity, URL, viewport, transform, and health",()=>{
  const cases=[
    ...
    ["missing",f=>f.report.results.pop(),"120"],
