...
診断下書きは画面移動やPWA再起動後も同じ端末内で再開できます。schema v4は公開HTTPSの初回ホーム、オンライン6経路・機内モード4経路の区間取得量・表示状態・取得可能なJS heap観測と、保存後・機内モード・再接続後の3時点における教材キャッシュ状態を記録します。受領した記録の必須欄は `npm run validate:device-check -- <record.json>` で検査できます。成功しても実機操作の内容やβ公開可否を自動承認するものではありません。
