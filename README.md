...
脳梁候補も、原画像と全該当直交断を確認した局所1,605 voxelと追加1,596 voxel（合計3,201）を未ラベルへ戻し、対応する3D部品を同期しました。周辺皮質や組織間隙への明瞭なはみ出しの修正であり、帯状回・脳弓との分離などは未完了です。AI支援のプロジェクト採用で、専門家レビューではありません。[初回修復](CALLOSUM_LOCAL_REPAIR.md)・[追加修復](CALLOSUM_CORTICAL_FOLLOWUP_REPAIR.md)。The development branch removes 1,605 initially reviewed voxels and a further 1,596 (3,201 in total) from the provisional callosal label, with corresponding 3D updates. This corrects selected spillover into adjacent cortex and tissue gaps, not the entire boundary. Separation from the cingulate gyrus and fornix remains incomplete; this is AI-assisted project adoption, not expert review.
