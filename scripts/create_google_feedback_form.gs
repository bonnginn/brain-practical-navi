/**
 * 脳実習ナビ：β版フィードバック／共同制作フォーム生成スクリプト
 *
 * 使い方:
...
var CONFIG = {
  FORM_TITLE: '脳実習ナビ β版｜修正提案・共同制作フォーム',
  RESPONSE_SHEET_TITLE: '脳実習ナビ β版｜フォーム回答・運用管理',
  PROJECT_NAME: '脳実習ナビ',
...
function createBrainPracticalFeedbackForm() {
  ...
  form.addCheckboxItem()
    .setTitle('共同制作に関する確認')
    .setChoiceValues([
      '公開βへの参加希望であり、報酬・採用・継続参加は個別の合意がない限り保証されないことを理解しました。',
...
function buildDescription_() {
  return [
    CONFIG.PROJECT_NAME + 'は、脳解剖実習の予習・復習を補助する非営利の教育用β版です。',
