...
function expectedObservedViewport(viewport){return {innerWidth:viewport.width,innerHeight:viewport.height,clientWidth:viewport.width,clientHeight:viewport.height,visualViewport:{width:viewport.width,height:viewport.height,scale:1},screen:{width:800,height:600,availWidth:800,availHeight:600},devicePixelRatio:viewport.dpr,maxTouchPoints:viewport.touch?1:0,coarsePointer:viewport.coarse,hover:!viewport.coarse,touchEvent:viewport.touch}}
...
function writeArtifact(root,ref,bytes){const file=path.resolve(root,...ref.split("/"));if(!containedPath(root,file))throw new Error("artifact path escaped output root");fs.mkdirSync(path.dirname(file),{recursive:true});fs.writeFileSync(file,bytes);return artifactDescriptor(ref,bytes,ref.endsWith(".rgba")?"application/x-rgba8":"application/x-binary-mask")}
