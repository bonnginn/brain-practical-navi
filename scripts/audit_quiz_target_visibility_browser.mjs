...
export const EXPECTED_QUIZ_TARGET_COUNTS = Object.freeze({ section:17,surface:6,neurovascular:17,total:40 });
export const EXPECTED_QUIZ_TARGET_INVENTORY_SHA256 = "91b9b9cd4f14ccbe740643d3714ea03819f9dfee24aa52e67315d8127222773b";
// Covers target, format, options, and the render namespace/IDs used by this audit.
export const EXPECTED_QUIZ_VISIBILITY_OPTIONS_SHA256 = "bcf3e901268f265c60ad492b4b014548c941a0d51e022245c39d79fb5f41ea1f";
...
const LABEL_ASSET_SHA256="6744e7c0184436789f42c7107d05ead93cf36703bb36372df5f63b82a38f7b56";
...
function expectedObservedViewport(viewport){return {innerWidth:viewport.width,innerHeight:viewport.height,clientWidth:viewport.width,clientHeight:viewport.height,visualViewport:{width:viewport.width,height:viewport.height,scale:1},screen:{width:viewport.width,height:viewport.height,availWidth:viewport.width,availHeight:viewport.height},devicePixelRatio:viewport.dpr,maxTouchPoints:viewport.touch?1:0,coarsePointer:viewport.coarse,hover:!viewport.coarse,touchEvent:viewport.touch}}
...
function expectedMeshProvenance(entry,capture,sourceLoader,mode){return {builder:MESH_MASK_BUILDER,namespace:entry.namespace,activeLayer:entry.activeLayer,sourceMeshes:expectedMeshSources(entry,sourceLoader,mode),selectedIds:[...entry.expectedIds],hemisphere:entry.view==="medial"?"left":"both",transform:structuredClone(capture.transform),projection:{canvasWidth:capture.canvas.intrinsicWidth,canvasHeight:capture.canvas.intrinsicHeight,scale:entry.format==="neurovascular"?.88:1,clipPolicy:"canvas-bounds",cullPolicy:entry.format==="surface"?"conservative-no-depth":"disabled-no-depth"}}}
const decodedMeshCache=new Map;
function decodeBnm3(bytes){const key=sha256Bytes(bytes),cached=decodedMeshCache.get(key);if(cached)return cached;let raw=Buffer.from(bytes);if(raw[0]===0x1f&&raw[1]===0x8b)raw=gunzipSync(raw);if(raw.subarray(0,4).toString("ascii")!=="BNM3")throw new Error("mesh asset must be BNM3");const nv=raw.readUInt32LE(4),declared=raw.readUInt32LE(8),faceOffset=12+nv*32;if(faceOffset>raw.length||(raw.length-faceOffset)%12)throw new Error("invalid BNM3 length");const nf=(raw.length-faceOffset)/12;if(declared!==nf&&declared!==nf*3)throw new Error("invalid BNM3 face count");const result={vertices:new Float32Array(raw.buffer,raw.byteOffset+12,nv*3),regions:new Float32Array(raw.buffer,raw.byteOffset+12+nv*28,nv),faces:new Uint32Array(raw.buffer,raw.byteOffset+faceOffset,nf*3)};decodedMeshCache.set(key,result);return result}
function rebuildExpectedMeshMask(entry,capture,sourceLoader){
  const width=capture.canvas.intrinsicWidth,height=capture.canvas.intrinsicHeight,mask=new Uint8Array(width*height),ids=new Set(entry.expectedIds),rot=capture.transform.rotation,ax=rot.x*Math.PI/180,ay=rot.y*Math.PI/180,az=rot.z*Math.PI/180,cx=Math.cos(ax),sx=Math.sin(ax),cy=Math.cos(ay),sy=Math.sin(ay),cz=Math.cos(az),sz=Math.sin(az),m=[cz*cy-sz*sx*sy,sz*cy+cz*sx*sy,-cx*sy,-sz*cx,cz*cx,sx,cz*sy+sz*sx*cy,sz*sy-cz*sx*cy,cx*cy],scale=capture.transform.zoom*(entry.format==="neurovascular"?.88:1),edge=(a,b,x,y)=>(x-a[0])*(b[1]-a[1])-(y-a[1])*(b[0]-a[0]);
  for(const file of expectedMeshFiles(entry)){const mesh=decodeBnm3(sourceLoader(`public/atlas/${file}`)),selected=new Uint8Array(mesh.regions.length),project=index=>{const o=index*3,q0=mesh.vertices[o+2],q1=mesh.vertices[o]+16,q2=mesh.vertices[o+1],rx=m[0]*q0+m[3]*q1+m[6]*q2,ry=m[1]*q0+m[4]*q1+m[7]*q2;return [(rx/96*scale*.5+.5)*width,(.5-ry/96*scale*.5)*height]};for(let i=0;i<selected.length;i++)if(ids.has(Math.round(mesh.regions[i])))selected[i]=1;for(let f=0;f<mesh.faces.length;f+=3){const ia=mesh.faces[f],ib=mesh.faces[f+1],ic=mesh.faces[f+2];if(!selected[ia]&&!selected[ib]&&!selected[ic])continue;const a=project(ia),b=project(ib),c=project(ic),area=edge(a,b,c[0],c[1]);if(Math.abs(area)<1e-6)continue;const minX=Math.max(0,Math.floor(Math.min(a[0],b[0],c[0]))),maxX=Math.min(width-1,Math.ceil(Math.max(a[0],b[0],c[0]))),minY=Math.max(0,Math.floor(Math.min(a[1],b[1],c[1]))),maxY=Math.min(height-1,Math.ceil(Math.max(a[1],b[1],c[1])));for(let y=minY;y<=maxY;y++)for(let x=minX;x<=maxX;x++){const e0=edge(a,b,x+.5,y+.5),e1=edge(b,c,x+.5,y+.5),e2=edge(c,a,x+.5,y+.5);if((e0>=0&&e1>=0&&e2>=0)||(e0<=0&&e1<=0&&e2<=0))mask[y*width+x]=1}}}return mask
}
...
function expectedMaskProvenance(entry,capture){return {builder:"bbs1-screen-mask-v1",labelAsset:{path:LABEL_ASSET_PATH,sha256:LABEL_ASSET_SHA256,dims:[...LABEL_DIMS]},plane:entry.plane,position:entry.position,sourceIds:[...entry.expectedIds],viewTransform:{algorithm:"AtlasVolumeCanvas.viewTransform-v1",cssWidth:capture.canvas.cssRect.width,cssHeight:capture.canvas.cssRect.height,dpr:capture.canvas.dpr,zoom:capture.transform.zoom,pan:{...capture.transform.pan}}}}
function rebuildExpectedSectionMask(entry,capture,mode){
  ...
  const volume=loadPinnedLabelVolume(),ids=new Set(entry.expectedIds),[sw,sh]=sectionSize(volume.dims,entry.plane),cssWidth=capture.canvas.cssRect.width,cssHeight=capture.canvas.cssRect.height,fit=Math.min((cssWidth-10)/sw,(cssHeight-10)/sh),scale=fit*capture.transform.zoom,ox=(cssWidth-sw*scale)/2+capture.transform.pan.x,oy=(cssHeight-sh*scale)/2+capture.transform.pan.y,mask=new Uint8Array(width*height);
...
function validateCapture(capture,{phase,item,entry,baseUrl,loader,mode,sourceLoader},errors,prefix){
  ...
  if(!same(capture.question,expectedQuestion(item,entry)))add(errors,`${prefix}: DOM question identity mismatch`);
  ...
  const projection=visibility.projectionMask;
...
function validateResult(result,item,entry,context,errors,index){
  ...
  if(h1&&c&&h2){
    ...
    if(entry.format!=="section"&&loaded.H1.mask){const coverage=sectionCoverage(loaded.H1.mask,h1,c,result.captures.H1.canvas.intrinsicWidth,result.captures.H1.canvas.intrinsicHeight),precision=coverage.changedInside/(coverage.changedInside+coverage.changedOutside);if(!same(result.comparisons.meshCoverage,coverage)||coverage.changedInside<=0||coverage.coverageRatio<.5||precision<.95||coverage.outsideRatio>.05)add(errors,`${prefix}: mesh projection coverage/outside fabricated or below precision thresholds`)}
...
function validateQuizTargetVisibilityReportCore(report,{mode,rawArtifactLoader,sourceAssetLoader,preErrors=[]}){
  ...
  if(!isRecord(report))return {passed:false,ok:false,errors:["report must be object"],summary:{expectedCount:120,observedCount:0,uniqueCount:0,passedCount:0,allPassed:false}};
  ...
  if(!exactKeys(report,reportKeys))add(errors,"report keys invalid");
  ...
  const rows=Array.isArray(report.results)?report.results:[];if(rows.length!==120)add(errors,"results must contain exactly 120 rows");const keys=new Set(rows.map(row=>row?.key));if(keys.size!==rows.length)add(errors,"duplicate result keys");
  const imageRefs=[];for(const row of rows)for(const phase of PHASES)if(row?.captures?.[phase]?.canvas?.artifact?.ref)imageRefs.push(row.captures[phase].canvas.artifact.ref);if(imageRefs.length!==360||new Set(imageRefs).size!==360)add(errors,"canvas artifact refs must be exact and globally unique");
  ...
  const derived={expectedCount:120,observedCount:rows.length,uniqueCount:keys.size,passedCount:errors.length?0:120,allPassed:errors.length===0};if(!same(report.summary,derived))add(errors,"summary fabricated or stale");
  return {passed:errors.length===0,ok:errors.length===0,errors,summary:{expectedCount:120,observedCount:rows.length,uniqueCount:keys.size,passedCount:errors.length?0:120,allPassed:errors.length===0}};
}

function failedValidation(errors){return {passed:false,ok:false,errors,summary:{expectedCount:120,observedCount:0,uniqueCount:0,passedCount:0,allPassed:false}}}
...
export function createValidQuizTargetVisibilityFixture(){
  ...
  const report={schemaVersion:2,tool:QUIZ_TARGET_VISIBILITY_TOOL,provenance:"unit-fixture",status:"complete",generatedAt:"2026-08-23T00:00:02.000Z",startedAt:"2026-08-23T00:00:00.000Z",completedAt:"2026-08-23T00:00:01.000Z",baseUrl,environment:{os:{platform:"win32",release:"fixture"},nodeVersion:"v24.0.0",browser:{product:"HeadlessChrome/151.0.0.0",userAgent:"Mozilla/5.0 HeadlessChrome/151.0.0.0",protocolVersion:"1.3"}},dependency:expectedDependency(),source:{path:QUIZ_TARGET_VISIBILITY_SOURCE_PATH,inventorySha256:EXPECTED_QUIZ_TARGET_INVENTORY_SHA256,optionsSha256:EXPECTED_QUIZ_VISIBILITY_OPTIONS_SHA256},inventory:structuredClone(QUIZ_TARGET_VISIBILITY_INVENTORY),viewports:structuredClone(QUIZ_TARGET_VISIBILITY_VIEWPORTS),emulationCommands:expectedEmulationCommands(),matrix:structuredClone(matrix),results,summary:{expectedCount:120,observedCount:120,uniqueCount:120,passedCount:120,allPassed:true}};
...
export function runQuizTargetVisibilityBrowserAudit(){return {status:"incomplete",provenance:"live-browser",reason:"live CDP coordinator is not implemented at this checkpoint",expectedMatrixCount:120}}

function parseArgs(argv){const args={help:false,output:null};for(let i=0;i<argv.length;i++){const token=argv[i];if(token==="--help"||token==="-h"){args.help=true;continue}if(token==="--output"){if(!argv[i+1])throw new Error("--output requires a path");args.output=argv[++i];continue}throw new Error(`unknown option: ${token}`)}return args}
export function main(argv=process.argv.slice(2)){
  let args;try{args=parseArgs(argv)}catch(error){console.error(errorText(error));process.exitCode=2;return}
  if(args.help){console.log("Usage: node scripts/audit_quiz_target_visibility_browser.mjs --help");console.log("Live collection is incomplete; every non-help invocation exits non-zero.");return}
  const result={...runQuizTargetVisibilityBrowserAudit(),sourceAudit:auditQuizTargetVisibilitySource(),environment:{platform:process.platform,nodeVersion:process.version,release:os.release()}};const text=`${JSON.stringify(result,null,2)}\n`;if(args.output){fs.mkdirSync(path.dirname(path.resolve(args.output)),{recursive:true});fs.writeFileSync(path.resolve(args.output),text,"utf8")}process.stdout.write(text);process.exitCode=2;
}
