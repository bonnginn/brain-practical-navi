...
def main():
    ...
    # These former comparison/prototype outputs are not read by the application.
    # Removing stale copies here prevents a local rebuild from silently adding
    # more than 100 MiB to the public deployment directory.
    for obsolete in ["mni-cerebra-1mm.bin","bigbrain-400um.bin.gz","brain.mesh","segment-cortex.mesh","brain-practical-segmented-v2.glb"]:
    ...
    for name,values,step,sigma in [("segment-cerebellum",cerebellum,1,.20),("segment-brainstem",brainstem,1,.20),("segment-deep",deep,1,.15),("segment-ventricles",ventricles,1,.10)]:write_mesh(OUT/f"{name}.mesh",np.isin(labs,list(values)),step=step,sigma=sigma)
