...
const FORBIDDEN_PREFLIGHT_CALLS = [
  ...
  "setTitle(", "setDescription(", "setChoiceValues(", "setChoices(", "setRequired(",
...
function parseEmbeddedArray(source, key) {
  ...
  return match[1].split(",").map(value => value.trim()).filter(Boolean).map(value => {
    ...
    throw new Error(`unsupported embedded ${key} value: ${value}`);
  });
}

export function validateEmbeddedPreflightSubset(contract, source) {
  const errors = [];
  const expected = {
    pageTitles: contract.form.pages.filter(page => page.key !== "route").map(page => page.title),
    itemTitles: contract.form.items.map(item => item.title),
    itemTypes: contract.form.items.map(item => item.type),
    required: contract.form.items.map(item => item.required),
  };
  for (const [key, values] of Object.entries(expected)) {
    let embedded;
    try { embedded = parseEmbeddedArray(source, key); } catch (error) { errors.push(error.message); continue; }
    if (JSON.stringify(embedded) !== JSON.stringify(values)) errors.push(`embedded ${key} differs from feedback-form-contract.json`);
  }
  if (!source.includes(`title: '${contract.form.title}'`)) errors.push("embedded form title differs from feedback-form-contract.json");
...
export function validateFeedbackFormContract(contract) {
  ...
  if (pages.find(page => page.key === "feedback")?.navigationAfterPage !== "SUBMIT") errors.push("feedback page must submit instead of falling through to collaboration");
  ...
  for (const [index, item] of items.entries()) {
    ...
    if (item.type === "MULTIPLE_CHOICE" || item.type === "CHECKBOX") {
      if (!Array.isArray(item.choices) && !item.choicesSource) errors.push(`${item.key}: choices are required`);
    }
  }
...
export function validatePreflightSource(source) {
  ...
  for (const required of ["FormApp.openById(", "SpreadsheetApp.openById(", "getItems(", "getTitle(", "getDescription(", "getPageNavigationType(", "getGoToPage(", "getValues(", "mismatchCodes", "contractSha256"] ) {
...
export function auditFeedbackFormPreflight({rootDir = REPOSITORY_ROOT, contract, preflightSource} = {}) {
  ...
  const embeddedResult = validateEmbeddedPreflightSubset(loadedContract, source);
