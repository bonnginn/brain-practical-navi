...
def main():
    ...
    target=cc==85;selected=np.argwhere(target)
    if len(selected)!=1314 or selected.min(0).tolist()!=[205,289,176] or selected.max(0).tolist()!=[216,334,210]:
        raise ValueError('Fixed reviewed component changed')
    out=ROOT/'work/anatomy-review/callosum-cortical-spillover-component85-v1';out.mkdir(parents=True,exist_ok=True)
    evidence=dict(imageSha256=EXPECTED_IMAGE_SHA256,labelsSha256=LABEL_SHA,sourceReportSha256=hashlib.sha256((source/'report.json').read_bytes()).hexdigest(),sourceSamplesSha256=hashlib.sha256((source/'sampled-callosal-classes.npz').read_bytes()).hexdigest(),component=85,count=1314,min=selected.min(0).tolist(),max=selected.max(0).tolist(),labelMutation=False,adopted=False,figures=[])
