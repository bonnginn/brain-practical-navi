...
- Web版をPWA化。アプリシェルと閲覧済み教材を自動キャッシュし、脳表26.4 MiB／断面39.3 MiB／局所標本18.8 MiBを選択保存できる管理画面を追加。Windows Chromiumで局所標本56ファイル保存後、通信遮断下の後脳3D再読込に合格。保存版・完了状態、接続・Storage Persistence・インストール状態を表示し、旧版の更新要求と共有教材を壊さない削除も実キャッシュで回帰済み。
