...
- TypeScript: 合格
- 本番Viteビルド: 合格
- `npm run audit:beta`: 12監査合格
- Go / No-Go: ローカル合格4条件、外部証拠待ち6条件
- 結論: **No-Go（β候補のローカル検証中）**

β候補の昇格は、少なくとも専門家レビュー、実スマートフォン、公開URLの証拠が揃った後に、プロジェクト管理者が判断します。
...
- 作業ツリーはクリーン
- `38890d3` 以降のコミットが存在
- `audit:beta` は12監査に合格し、4ローカル合格／6外部証拠待ち、No-Goを表示
- テストと本番ビルドが合格
...
- 保存済み判定へHTTP状態、HTMLフォールバック除外、Content-Length一致を追加。欠損atlasがSPA HTMLとしてHTTP 200になる条件を正常教材と誤認しない。Windows実ブラウザでは3セット保存済み、部分保存0、console error 0を維持。
- Service Workerの通常閲覧キャッシュにも同じ健全性検査を追加し、明示保存済み教材をネットワークより先に返すよう変更。previewサーバーを実際に停止して脳表、水平断、後脳標本、標準クイズを完全再読込し、全Canvas描画、クイズ回答、console error 0を確認。Worker生成ロジック変更でもキャッシュ世代が更新される。
- CSS viewport 907 pxのPC分割表示で教材ナビと補助操作が重なる回帰を検出・修正。761–1199 pxは電話UIへ切り替えず二段ヘッダーとし、760 / 761 / 907 / 1024 / 1199 / 1200 / 1366 pxで重なり・横はみ出し0を実測。1024 pxトップの同梱QRも再確認した。
- 実機診断の性能記録はHistory API移動後・同一端末の別タブ・再読込後も直前教材URLを保持し、9経路を所定順・教材種別・接続状態一致の場合だけ保存する。編集ツールはPC向けのためスマートフォン必須経路から外した。途中項目の解除では後続観測も連鎖解除し、validatorで最後に失敗する証拠を画面内で作りにくくした。
...
- スマートフォンUIは5教材ワークスペース下部ドック、画面別設定シート、安全域、フォーカス循環・復帰、背景スクロール停止を実装。共同制作向け編集ツールはPC専用。
...
- Form、Issue、PRの用途を分離。公開Formは未ログインで3分岐を確認し、2026-08-15にテスト回答の送信、回答表とFormsからの二重削除、0件への復帰に成功。
- 共同制作条件を管理者が確認し、`CONTRIBUTING.md` を正式版へ更新。
...
| 8 | Form・Issue・PR導線 | ローカル合格 | 公開反映後の導線再確認 |
| 9 | 神経解剖学専門家レビュー | 専門家待ち | 少なくとも1名の専門家を含む19/19のv2記名JSON、未解決指摘の修正、台帳・画面反映 |
| 10 | 未完成項目の公開 | 公開待ち | β候補公開URLで既知の制限を確認 |

詳細と証拠は `BETA_GATE_AUDIT.md` が正本です。`npm run audit:beta` は11のローカル監査とこの状態区分を再検証しますが、外部証拠待ちを完了へ変更しません。

試作クイズの標準昇格は `QUIZ_REVIEW_LEDGER.md` と `app/quiz-review-ledger.json` で管理します。現在の承認は0件です。19/19の専門家レビュー、神経解剖学の専門性、未解決判定なし、個別画面との対応、管理上の採用理由が揃うまで試作問題は既定OFFのままです。

## 6. 次に進める順序

1. 実スマートフォンでトップ、脳表、断面、局所標本、クイズ、専門家票を完走し、CPU／GPU、タッチ、OS文字サイズ、安全域、回線、ピークメモリを記録する。編集ツールはPCで別確認する。
2. 神経解剖学の確認者へ `EXPERT_REVIEW_CHECKLIST.md` と40桁SHA付き固定URLを渡し、19対象のv2記名JSONを受領する。一括検証後、指摘の修正と台帳・画面反映を別変更で行う。Codexは判定内容を代筆しない。
3. 明示的な許可後にだけbeta branchをpushし、公開URLとGitHub Actionsを巡回する。公開・`main`統合は別判断。
4. 専門家・学習者のM2比較記録から、次の比較課題または採用基盤を決定する。
