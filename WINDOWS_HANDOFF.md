...
追記: M2比較の方法検証として、`/?m2=compare#workspace/blocks/hindbrain` に標本再構成中心とプロジェクト模式中心の同期比較を追加しました。13部品の由来・共通座標・4 MiB予算を `npm run audit:models` で固定し、1364 pxでは横並び、391 pxでは縦積み、2 Canvas読込エラー0を確認しています。βは標本再構成を基盤にし、模式形状を着脱補助とする暫定判断です。優先例と専門家・学習者比較は残るためM2全体は継続します。検証基準は73/73自動テスト、型検査、本番Viteビルド成功です。詳細は [MODEL_COMPARISON_AUDIT.md](MODEL_COMPARISON_AUDIT.md) を参照してください。
