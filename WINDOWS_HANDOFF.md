...
- 実機診断の性能記録はHistory API移動後も直前教材URLを保持し、10経路を所定順・教材種別・接続状態一致の場合だけ保存する。途中項目の解除では後続観測も連鎖解除し、validatorで最後に失敗する証拠を画面内で作りにくくした。
...
3. 管理者が `CONTRIBUTING.md` の「公開前ドラフト」を正式版へするか判断する。
4. 明示的な許可後にだけbeta branchをpushし、公開URLとGitHub Actionsを巡回する。公開・`main`統合は別判断。
5. 専門家・学習者のM2比較記録から、次の比較課題または採用基盤を決定する。
