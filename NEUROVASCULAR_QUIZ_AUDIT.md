...
最新の追補：Vの側頭葉組織内への経路も原画像で確認しcn5を保留へ追加。現在は保存22／出題18（血管9・神経9）、全bank保存100／出題92、保留8問です。以下の19／94・3対象はIX〜XIだけを保留した直前段階の記録です。

## 2026-09-06 開発版の出題保留（未公開）

下記22問は保存インベントリです。現在は小脳組織内への模式経路の貫入を確認したcn9/cn10/cn11を出題から保留し、名称同定pilotの出題可能数は19問（血管9・神経10）です。各神経の機能問題も含めた保留は6問で、全保存100問に対し出題可能94問です。名称の文字選択肢は他問題に残ります。問題データと元のインベントリSHAは削除・改変していません。

`audit_neurovascular_quiz.mjs` は保存数をsummary、出題可能数をeligibilityへ分離し、保留3対象の一致、問題種別結合後のruntime filter、誤答履歴からの復活防止を検査します。これは解剖学的経路の修正完了ではありません。根拠はNERVE_ORIGIN_IMAGE_REVIEW.md参照。
