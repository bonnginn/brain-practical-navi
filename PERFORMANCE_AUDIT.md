...
診断チェックリスト側でも、10経路の順序、現在の教材種別、接続状態を記録前に検査します。未実施の前項目より後は無効化し、途中項目を解除した場合はそれ以降のResource Timing・heap観測も削除します。誤った経路を一度保存して最後のvalidatorで初めて失敗するのではなく、実機操作中に修正できるようにしました。
